...
- **인사이트 우선(결론 히어로)**: 카드/행은 raw 데이터가 아니라 "그래서 뭐(so-what)"를 히어로로. 뉴스=시사점>제목, 경쟁사=한 줄 결론 콜아웃을 표보다 위, KPI=값+변화(▲▼·%)를 같은 줄에 co-location(방향은 컬러로 즉시).
- **정보 tier 분리**: 배지 → 결론/타이틀 → 핵심수치 → 보조메타 → 액션을 각각 독립 줄로(한 줄 크램 금지).
- **신뢰·최근성 신호 표면 노출**: 신뢰도(C/확인·AI)·출처·"n일 전"을 모달/각주에 숨기지 말고 카드 면에 작지만 상시. 액션과 구분선/여백으로 분리.
- **클릭 어포던스 통일**: 상세로 열리는 카드는 hover lift + 우측 `자세히 ›`(chevron, group-hover indigo·translate). 외부 원문 링크는 별개 액션으로 유지.
- **그리드 여백**: 카드 그리드 `gap-4` 기준(밀도 유지하되 내부는 숨쉬게). 밀집 4열은 `xl`에서만.
