    ...
    <div className="flex flex-col gap-4">
      ...
      <section className="min-w-0 rounded-xl p-4" style={{ animation: "fadeUp .34s cubic-bezier(.16,1,.3,1) both" }}>
        ...
        <header className="mb-3 flex flex-wrap items-center gap-2.5 border-b border-gray-100 dark:border-gray-800 pb-2.5">
          ...
          <h2 className="text-[17px] font-bold tracking-tight text-gray-900 dark:text-gray-50">가전 수입 단가</h2>
          <Segmented value={hs} onChange={setHs} options={HS.map((h) => ({ k: h.hs, label: h.label }))} size="sm" />
          <span className="text-[12px] font-semibold text-gray-400 dark:text-gray-500">HS {hs}{lastDate ? " · 최신 확보 " + fmtMonth(lastDate) : ""}</span>
        ...
        {chart.series.length ? (
          ...
          <div className="flex h-[300px] flex-col items-center justify-center rounded-xl border border-dashed border-gray-200 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-900/60 text-[13px] text-gray-400">{Object.keys(ser).length ? "해당 품목 데이터 부족" : "불러오는 중"}</div>
        )}

        {/* 원산지 점유·단가 */}
        <div className="flex h-full flex-col rounded-xl p-3.5" style={{ animation: "fadeUp .34s cubic-bezier(.16,1,.3,1) both" }}>
          <div className="flex items-center gap-1.5">
            ...
            {orig && <span className="ml-auto text-[11.5px] text-gray-400 dark:text-gray-500">{fmtMonth(orig.date)}</span>}
          </div>
          <div className="mt-2.5 min-h-0 flex-1">
            {!orig ? <div className="flex h-40 items-center justify-center text-[13px] text-gray-400">불러오는 중</div> : orig.origins.length === 0 ? <div className="flex h-40 items-center justify-center text-[13px] text-gray-400">원산지 데이터 없음</div> : (
              <div className="flex flex-col gap-2">
                {orig.origins.slice(0, 7).map((o) => (
                  <div key={o.partner} className="flex items-center gap-2">
                    <span className="w-14 shrink-0 text-[13px] font-semibold text-gray-700 dark:text-gray-200">{o.partner}</span>
                    <div className="relative h-5 flex-1 overflow-hidden rounded bg-gray-100 dark:bg-gray-800">
                      ...
                      <span className="absolute inset-y-0 left-2 flex items-center text-[11.5px] font-bold text-gray-700 dark:text-gray-100">{o.share.toFixed(0)}%</span>
                    </div>
                    <span className="w-16 shrink-0 text-right text-[12px] tabular-nums text-gray-500 dark:text-gray-400">${o.unit.toFixed(2)}/kg</span>
                  </div>
                ))}
              </div>
            )}
          </div>
          <p className="mt-2 text-[12px] leading-relaxed text-gray-500 dark:text-gray-400"><b className="font-semibold text-gray-700 dark:text-gray-200">의미</b> 막대=수입액 점유, 우측=원산지별 단가($/kg) — <b className="text-gray-700 dark:text-gray-200">중국 저가 대량·한국 고단가</b> 구도 확인</p>
          <p className="mt-auto pt-2 text-[11px] text-gray-400 dark:text-gray-500">UN Comtrade · 최신 확보월 원산지별 CIF 점유·단가</p>
        </div>
      </div>
      </section>
      ...
      <p className="text-[12px] leading-relaxed text-gray-400 dark:text-gray-500">UN Comtrade 필리핀 수입 통계(HS 8418 냉장고·8415 에어컨·8450 세탁기·8528 TV) · 단가=CIF÷순중량($/kg) · 무역데이터 특성상 발표 지연(최신 확보월 기준) · 연간 수입액은 &lt;가전 선행지표&gt; 참조</p>
    </div>
  )
}
