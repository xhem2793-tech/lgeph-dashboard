    ...
    <div className="grid gap-3 lg:grid-cols-[1.5fr_1fr]">
      ...
      <section className="min-w-0 overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-sm" style={{ animation: "fadeUp .5s ease both" }}>
        <div className="flex flex-wrap items-center gap-2.5 border-b border-gray-100 dark:border-gray-800 pb-2.5">
          ...
          <h2 className="text-[15px] font-bold tracking-tight text-gray-900 dark:text-gray-50">지역 × 품목 물가</h2>
          <span className="text-[11px] font-semibold text-gray-400 dark:text-gray-500">전년비 % · 물가 높은 순 · 지역 물가 상세</span>
        </div>
          ...
          <div className="grid min-w-[440px] gap-1" style={{ gridTemplateColumns: "auto repeat(6,1fr)" }}>
            ...
            {rows.map((r, ri) => (
              <React.Fragment key={ri}>
                <div className="flex items-center justify-end pr-2 text-right text-[11px] text-gray-600 dark:text-gray-300">{String(r.geo).replace(/\(.*/, "").replace("Region ", "").trim().slice(0, 12)}</div>
                {cols.map(([, key], ci) => {
                  const v = num(r[key])
      ...
      <section className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-sm" style={{ animation: "fadeUp .5s ease both" }}>
        <h2 className="text-[15px] font-bold text-gray-900 dark:text-gray-50">지역 물가 분포 <span className="text-[11px] font-normal text-gray-400 dark:text-gray-500">전국 {rows.length}곳</span></h2>
        <div className="mt-3 flex gap-5">
          <div><p className="text-[11px] text-gray-400 dark:text-gray-500">평균</p><p className="text-[22px] font-bold tabular-nums text-gray-900 dark:text-gray-50">{rMean.toFixed(1)}<span className="text-[12px] text-gray-400 dark:text-gray-500">%</span></p></div>
          <div><p className="text-[11px] text-gray-400 dark:text-gray-500">범위</p><p className="text-[22px] font-bold tabular-nums text-gray-900 dark:text-gray-50">{rLo.toFixed(1)}–{rHi.toFixed(1)}</p></div>
        </div>
