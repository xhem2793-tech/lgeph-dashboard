...
export function AnomalyView({ rows, stamp }: { rows: PriceRow[] | null; stamp: string | null }) {
  ...
  return (
    <div className="mt-3 flex flex-col gap-3">
      ...
      {/* 피드 */}
      {days.length === 0 ? (
        <p className="rounded-xl py-16 text-center text-[12.5px] text-gray-400 dark:text-gray-500">감지된 이상치가 없습니다.</p>
      ) : days.map((g) => (
        <div key={g.dk}>
          <div className="mb-2 mt-1 flex items-center gap-2">
            <span className="text-[12.5px] font-bold text-gray-800 dark:text-gray-100">{g.label}</span>
            ...
            {g.items.map((a, i) => { const sv = SEV_META[a.sev]; return (
              <div key={a.id + i} className="flex items-center gap-3 border-b border-gray-100 dark:border-gray-800/60 px-3 py-2.5 transition-colors last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800/40">
                ...
                <div className="flex min-w-0 flex-1 items-baseline gap-2">
                  <span className="shrink-0 whitespace-nowrap text-[12.5px] font-bold text-gray-900 dark:text-gray-50">{a.brand === "LG" ? <span className="text-indigo-700 dark:text-indigo-300">{a.title}</span> : a.title}</span>
