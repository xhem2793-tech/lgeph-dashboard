...
export function AnomalyView({ rows, stamp }: { rows: PriceRow[] | null; stamp: string | null }) {
  ...
  return (
    <div className="mt-3 flex flex-col gap-3">
      ...
      {/* 피드 */}
      {days.length === 0 ? (
        <p className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 py-16 text-center text-[14px] text-gray-400 dark:text-gray-500">감지된 이상치가 없습니다.</p>
      ) : days.map((g) => (
        <div key={g.dk}>
