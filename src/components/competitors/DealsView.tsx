  ...
  return (
    <div className="mt-3 overflow-hidden rounded-xl">
      <header className="flex flex-wrap items-center gap-x-2 gap-y-1 border-b border-gray-100 dark:border-gray-800 px-4 py-3">
        ...
        <span className="rounded border border-rose-200 dark:border-rose-500/30 bg-rose-50 dark:bg-rose-500/10 px-1.5 py-0.5 text-[11px] font-bold tracking-wide text-rose-600 dark:text-rose-400">INTERNAL USE ONLY</span>
        <div className="ml-auto flex items-center gap-1"><span className="mr-0.5 text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-gray-500">종류</span>{PTYPES.map((p) => <span key={p.k} className={"rounded px-1.5 py-0.5 text-[11.5px] font-bold " + p.cls}>{p.label}</span>)}</div>
      </header>
      {/* 필터 */}
      <div className="flex flex-col gap-2.5 border-b border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-900/40 px-4 py-3">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
          ...
          <div className="relative ml-auto">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="모델·브랜드 검색" className="w-[200px] rounded-full border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 py-1.5 pl-8 pr-3 text-[13.5px] outline-none focus:border-indigo-400 dark:focus:border-indigo-500/50 focus:shadow-[0_0_0_3px_rgba(99,102,241,0.12)]" />
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
          <span className="w-9 shrink-0 text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-gray-500">가격대</span>
          <span className="w-[74px] shrink-0 text-right text-[13px] font-bold tabular-nums text-indigo-700 dark:text-indigo-300">{peso(rmin)}</span>
          ...
          <span className="w-[74px] shrink-0 text-[13px] font-bold tabular-nums text-indigo-700 dark:text-indigo-300">{peso(rmax)}</span>
          <button type="button" onClick={() => { setRmin(dom[0]); setRmax(dom[1]) }} className="rounded-md px-2 py-1 text-[12px] font-semibold text-gray-500 dark:text-gray-400 ring-1 ring-inset ring-gray-200 dark:ring-gray-700 transition hover:bg-white dark:hover:bg-gray-800">초기화</button>
          <p className="ml-auto text-[12.5px] text-gray-500 dark:text-gray-400">{lgRef ? <>자사 <b className="text-indigo-700 dark:text-indigo-300">{lgRef.model}</b> 기준</> : "자사(LG) 모델 없음"} · 표시 <b className="tabular-nums">{list.length}</b></p>
        </div>
      </div>
      {/* 테이블 */}
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1200px] table-fixed border-collapse text-[13.5px]">
          <colgroup>
          ...
          <tbody>
            {list.length === 0 ? (
              <tr><td colSpan={11} className="px-4 py-10 text-center text-[13.5px] text-gray-400 dark:text-gray-500">선택한 조건에 해당하는 제품이 없습니다.</td></tr>
              ...
              return (
                <tr key={o.cc + ri} className={"border-b border-gray-100 dark:border-gray-800/60 last:border-0 " + (o.own ? "bg-indigo-50/40 dark:bg-indigo-500/5" : "hover:bg-gray-50 dark:hover:bg-gray-800/40")}>
                  ...
                  <td className={"px-3 py-3 " + (o.own ? "border-r border-indigo-100 dark:border-indigo-500/20" : "")}>
                    <span className="flex flex-wrap items-center gap-1.5">{o.own ? <span className="h-3.5 w-1 rounded bg-indigo-500" /> : null}<span className={"text-[14px] font-bold " + (o.own ? "text-indigo-700 dark:text-indigo-300" : "text-gray-800 dark:text-gray-100")}>{o.brand}</span>{o.own ? <span className="rounded bg-indigo-100 dark:bg-indigo-500/20 px-1 py-px text-[11px] font-bold text-indigo-600 dark:text-indigo-300">자사</span> : isBest ? <span className="rounded bg-emerald-500 px-1 py-px text-[11px] font-bold text-white">최저가</span> : null}</span>
                    <span className={"mt-0.5 block " + (o.own ? "pl-2.5" : "")}>{o.url ? <a href={o.url} target="_blank" rel="noopener noreferrer" className="text-[12px] tabular-nums text-indigo-600 hover:underline dark:text-indigo-400">{o.model}</a> : <span className="text-[12px] tabular-nums text-gray-400">{o.model}</span>}</span>
                  </td>
                  <td className="px-3 py-3 text-[13.5px] text-gray-600 dark:text-gray-300">{o.at ? pmShopLabel(o.at) : "—"}</td>
                  <td className="px-3 py-3 text-right"><span className={"text-[15px] tabular-nums font-bold " + (o.own ? "text-indigo-700 dark:text-indigo-300" : isBest ? "text-emerald-700 dark:text-emerald-300" : "text-gray-900 dark:text-gray-50")}>{won(o.net)}</span>{o.list != null && o.list > o.net ? <div className="text-[11.5px] tabular-nums text-gray-400 line-through dark:text-gray-500">{won(o.list)}</div> : null}</td>
                  <td className="px-3 py-3 text-center">{disc > 0 ? <span className="rounded bg-rose-50 dark:bg-rose-500/10 px-1.5 py-0.5 text-[12px] font-bold tabular-nums text-rose-600 dark:text-rose-400">-{disc}%</span> : <span className="text-gray-300 dark:text-gray-600">—</span>}</td>
                  <td className="px-3 py-3 text-right text-[13.5px] tabular-nums">{gapCell(o)}</td>
                  {PTYPES.map((p) => { const pa = promoAt(o, p.k); return <td key={p.k} className="border-l border-gray-100 dark:border-gray-800 px-3 py-3 align-top">{pa ? <><span className={"inline-flex items-center whitespace-nowrap rounded px-1.5 py-0.5 text-[12.5px] font-semibold " + p.cls}>{pa.val}</span>{pa.other ? <span className="mt-0.5 block text-[11px] text-amber-600 dark:text-amber-400">↳ {pmShopLabel(pa.at)}</span> : null}</> : <span className="text-[14px] text-gray-300 dark:text-gray-600">—</span>}</td> })}
                </tr>
              )
            })}
          </tbody>
      ...
      <div className="border-t border-gray-100 dark:border-gray-800 px-4 py-3">
        {lgRef ? (
          <p className="flex items-start gap-1.5 text-[13px] leading-relaxed">
            <span className={"mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-[11.5px] font-bold text-white " + (win ? "bg-emerald-600" : "bg-rose-500")}>LG 시사점</span>
        ...
        ) : <p className="text-[13px] text-gray-400 dark:text-gray-500">이 유형·용량에 자사(LG) 모델이 없어 vs 자사 비교를 생략합니다.</p>}
      </div>
    </div>
  )
}
