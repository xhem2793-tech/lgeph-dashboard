...
// 스크래핑 커버리지 히트맵 — 가로=거래선, 세로=브랜드. 셀=선택일 전시(스크랩)된 제품 수(=pN 존재).
// 각 셀: 당일 수(색농도) · 어제대비 Δ · 신규(어제 없던 것) · 품절(오늘 OOS). 상단 날짜 토글. (지역지도 히트맵 스타일)
...
export function MoversView({ rows, elabels, stamp }: { rows: PriceRow[] | null; elabels: EnergyRow[] | null; stamp: string | null }) {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      {/* 스크래핑 커버리지 히트맵 — 거래선×브랜드 전시 제품 수(날짜 토글·어제대비·신규·품절) */}
      <CoverageHeatmap rows={R} />
