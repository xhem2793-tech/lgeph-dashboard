...
function MvSpark({ series }: { series: number[] | null }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 align-middle">
      ...
      <span className={"text-[11px] font-semibold tabular-nums " + (dn ? "text-emerald-600 dark:text-emerald-400" : up ? "text-rose-600 dark:text-rose-400" : "text-gray-400 dark:text-gray-500")}>{chg > 0 ? "+" : ""}{chg.toFixed(1)}%</span>
    </span>
  )
}
  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      <div className="relative z-20 flex flex-wrap items-center gap-x-3 gap-y-2 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-900/40 px-3 py-2.5">
        ...
        <div className="relative flex items-center rounded-full border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-0.5 text-[12px] font-semibold shadow-sm">
          ...
          <tbody>
            {list.length === 0 ? (
              ...
              <tr key={r.retailer + r.model + ri} style={{ animation: "rowIn .32s ease both", animationDelay: Math.min(ri, 20) * 0.018 + "s" }} className="border-b border-gray-50 dark:border-gray-800/50 transition-colors hover:bg-indigo-50/50 dark:hover:bg-indigo-500/5">
                ...
                <td className="px-2 py-1.5 text-center text-[11.5px] text-gray-500 dark:text-gray-400">{r.category}</td>
                ...
                <td className="px-1 py-1.5 text-center">{star != null ? <span className={"rounded px-1 text-[10px] font-bold " + pmStarCls(star)}>★{star}</span> : <span className="text-gray-300 dark:text-gray-600">·</span>}</td>
                ...
                <td className="whitespace-nowrap border-l border-gray-100 dark:border-gray-800 px-2 py-1.5 text-center text-[12px] text-gray-500 dark:text-gray-400">{pmShopLabel(r.retailer)}</td>
              </tr>
              )
            })}
          </tbody>
      ...
      <p className="text-[11px] text-gray-400 dark:text-gray-500">★=New DOE 에너지등급 · SRP=권장소비자가 · 할인율=SRP 대비 · 오늘·날짜열=최근 4일(3일전까지) 실판매가 · 전일비=(오늘−어제)/어제, ↓ 인하(초록)·↑ 인상(빨강)·₱↔% 토글 · 최근 7일 변동=7일 추이 스파크라인 · {effCat} · 오늘 변동 {moved}건 · 상위 400건</p>
    </div>
  )
}
