  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      <div className="relative z-20 flex flex-wrap items-center gap-x-3 gap-y-2 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-900/40 px-3 py-2.5">
        ...
        <div className="w-fit"><PmDrop label={T("제품", "Div")} sel={cat} options={cats.map((c) => ({ k: c, t: catLabel(c) }))} onSelect={(k) => { setCat(k); setForm("전체"); setSize("전체") }} /></div>
      ...
      <div className="max-h-[1040px] overflow-auto rounded-lg border border-gray-200 dark:border-gray-800">
        <table className="w-full min-w-[1360px] table-fixed border-collapse text-[12px]">
          <colgroup>
            <col style={{ width: 138 }} /><col style={{ width: 58 }} /><col style={{ width: 70 }} /><col style={{ width: 128 }} /><col style={{ width: 32 }} /><col style={{ width: 100 }} />
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={BOARD_SHOPS.length + 9} className="px-3 py-12 text-center text-gray-400 dark:text-gray-500">{T("불러오는 중…", "Loading…")}</td></tr>
            ) : data.length === 0 ? (
              <tr><td colSpan={BOARD_SHOPS.length + 8} className="px-3 py-12 text-center text-gray-400 dark:text-gray-500">{T("조건에 맞는 모델 없음", "No models match the filters")}</td></tr>
