...
import { peso, pmShopLabel, pmStarCls, pmTierLabel, pmTierBar, DOE_CODE, doeNorm, pmMean, pmTicks, pmShort, PmDrop, ListSearch, catLabel } from "@/components/competitors/shared"
        ...
        <div ref={cardRef} className="overflow-hidden rounded-xl" style={{ minWidth: minW }}>
          <header className="flex flex-wrap items-center gap-x-2 gap-y-1 border-b border-gray-100 dark:border-gray-800 px-4 py-2.5">
            ...
            {(() => { const sp = effSpec !== "전체" ? effSpec : effForm !== "전체" ? effForm : ""; return (
              <span className="text-[22.5px] font-bold leading-tight text-gray-800 dark:text-gray-100">{catLabel(cat)} <span className="font-semibold text-gray-400 dark:text-gray-500">{T("vs 경쟁사", "vs Competitors")}</span> <span className="text-indigo-600 dark:text-indigo-400">{sp ? sp + " " : ""}{T("가격", "Price")}</span></span>
            ) })()}
            <div className="ml-auto flex items-center gap-1.5">
              ...
              <button type="button" onClick={dlPng} disabled={dling} data-noexport="1" aria-label={T("이미지 다운로드", "Download image")} title={dling ? T("생성중…", "Generating…") : T("카드 전체를 PPT 슬라이드 크기 이미지로 저장", "Save the full card as a PPT slide-sized image")} className="flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-500 dark:text-gray-400 transition-all duration-200 hover:border-indigo-300 dark:hover:border-indigo-500/40 hover:text-indigo-600 dark:hover:text-indigo-400 active:scale-95 disabled:opacity-50">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" className={dling ? "animate-pulse" : ""}><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><path d="M7 10l5 5 5-5" /><path d="M12 15V3" /></svg>
              </button>
            </div>
          </header>
