  ...
  return (
    ...
    <div className="flex h-full flex-col rounded-xl bg-[#f9fafb] p-3.5 transition-all duration-300 ease-out hover:-translate-y-1 hover:bg-white dark:hover:bg-gray-900 hover:shadow-[0_12px_34px_-12px_rgba(99,102,241,0.4)]">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-2.5 text-[13px] text-gray-400 dark:text-gray-500">
      ...
      {multi ? (
        <div className="mt-1.5 flex flex-nowrap gap-1 overflow-hidden">
          {items.map((it, i) => (
            <button key={i} onClick={() => setSel(i)} className={"shrink-0 whitespace-nowrap rounded px-1.5 py-0.5 text-[12px] font-medium transition-all duration-200 active:scale-95 " + (idx === i ? "bg-indigo-600 text-white shadow-sm" : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:-translate-y-0.5 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-indigo-600 dark:hover:text-indigo-400")}>{it.label}</button>
          ))}
        </div>
      ) : null}
      <p className="mt-1 flex items-baseline gap-1.5">
        ...
        <span className="cursor-default text-[12px] text-gray-400/90 dark:text-gray-500/90">{s.prevText}</span>
      </p>
      ...
      <div className="mt-auto border-t border-gray-200 dark:border-gray-800 pt-2">
        <span className="cursor-default text-[12px] text-gray-400 dark:text-gray-500">{t("source")} {s.note}</span>
      </div>
    </div>
    </div>
  )
          ...
          <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
                <div className="flex items-baseline gap-2">
                  ...
                  <div className="relative flex shrink-0 rounded-full border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 p-0.5">
                    ...
                    {RANGES.map((r, i) => (
                      <button key={r.key} ref={(el) => { rangeBtns.current[i] = el }} onClick={() => setRange(r.key)} className={"relative z-10 rounded-full px-2.5 py-1 text-[13px] font-medium transition-colors duration-300 active:scale-95 " + (range === r.key ? "text-white" : "text-gray-600 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400")}>{t(("r_" + r.key) as "r_7d")}</button>
                    ))}
                  </div>
                </div>
                  <span className="hidden items-center gap-1.5 text-[12px] text-gray-400 dark:text-gray-500 sm:flex">
