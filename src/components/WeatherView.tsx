...
function ClimateChart({ mon }: { mon: { rain: number; hi: number }[] }) {
  if (mon.length < 12) return <div className="flex h-full items-center justify-center text-[14px] text-gray-400">데이터 부족</div>
...
function DailyChart({ days }: { days: WxDay[] }) {
  ...
  if (d.length < 3) return <div className="flex h-full items-center justify-center text-[14px] text-gray-400">데이터 부족</div>
...
function QuakeStrip({ quakes }: { quakes: Quake[] }) {
  ...
  if (!quakes.length) return <div className="flex h-32 items-center justify-center text-[14px] text-gray-400">지진 데이터 없음</div>
  ...
  return (
    <div className="flex h-full flex-col rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3.5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md" style={{ animation: "fadeUp .34s cubic-bezier(.16,1,.3,1) both" }}>
      <div className="flex items-center gap-1.5">
        ...
        {seg && <span className="shrink-0 rounded bg-indigo-50 dark:bg-indigo-500/10 px-1.5 py-0.5 text-[11px] font-bold text-indigo-700 dark:text-indigo-300">{seg}</span>}
      </div>
      <div className="mt-2 min-h-0 flex-1">{children}</div>
      <p className="mt-2 text-[13px] leading-relaxed text-gray-500 dark:text-gray-400"><b className="font-semibold text-gray-700 dark:text-gray-200">의미</b> {meaning}</p>
      <p className="mt-auto pt-2 text-[12px] text-gray-400 dark:text-gray-500">{src}</p>
    </div>
  )
}
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800 bg-gradient-to-br from-sky-50 via-white to-white dark:from-sky-500/10 dark:via-gray-900 dark:to-gray-900 p-4 shadow-sm" style={{ animation: "fadeUp .5s ease both" }}>
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-baseline gap-1.5">
                ...
                <span className="num text-[18px] font-semibold text-gray-400 dark:text-gray-500">/ {cur?.minT != null ? Math.round(cur.minT) : "–"}°</span>
              </div>
              <div className="mt-0.5 text-[13px] text-gray-500 dark:text-gray-400">필리핀(전국 관측) · {cur ? cur.date.slice(5).replace("-", "/") : "—"} · {season}</div>
            </div>
          </div>
          {/* 체감(열지수) */}
          <div className="flex flex-col">
            <span className="text-[12.5px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">체감(열지수)</span>
            <span className="flex items-center gap-1.5">
              <span className="num text-[23px] font-bold text-gray-900 dark:text-gray-50">{cur?.heat != null ? cur.heat.toFixed(1) + "°" : "–"}</span>
              <span className={"rounded px-1.5 py-0.5 text-[12px] font-bold " + hr.bg + " " + hr.c}>{hr.t}</span>
            </span>
          </div>
          {/* 타일 */}
          <div className="flex flex-wrap items-center gap-2">
            {[
              ...
              <div key={t.l} className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white/70 dark:bg-gray-900/70 px-2.5 py-1.5 text-center">
                <div className="text-[11.5px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">{t.l}</div>
                <div className={"num text-[15px] font-bold " + t.c}>{t.v}</div>
              </div>
            ))}
          </div>
          <div className="ml-auto hidden shrink-0 items-center gap-0.5 rounded-full border border-gray-200 dark:border-gray-800 bg-white/70 dark:bg-gray-900/70 p-0.5 sm:flex">
            {WIN.map((w) => (
              <button key={w.k} type="button" onClick={() => setWin(w.k)} className={"rounded-full px-2.5 py-0.5 text-[13px] font-semibold transition-colors " + (win === w.k ? "bg-sky-600 text-white shadow-sm" : "text-gray-500 dark:text-gray-400 hover:text-sky-600 dark:hover:text-sky-400")}>{w.k}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="grid items-stretch gap-4 lg:grid-cols-2">
        ...
        <Panel title="일별 기온·강수 (최근 90일)" seg="관측"
          ...
          <div className="h-[150px] w-full">{wx.length ? <DailyChart days={wx} /> : <div className="flex h-full items-center justify-center text-[14px] text-gray-400">불러오는 중</div>}</div>
        </Panel>
        ...
        <Panel title="월별 기후 평년 (우기·건기)" seg="기후"
          ...
          src="PAGASA 관측 월별 집계">
          <div className="h-[150px] w-full">{monthly.some((m) => m.hi > 0) ? <ClimateChart mon={monthly} /> : <div className="flex h-full items-center justify-center text-[14px] text-gray-400">불러오는 중</div>}</div>
        </Panel>
        ...
        >
          {phase ? (
            <div className="flex h-full flex-col">
              <div className="flex items-center gap-3">
                <span className={"flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[18px] font-extrabold " + phase.bg + " " + phase.c}>
                  <span className={"h-2.5 w-2.5 rounded-full " + phase.dot} />{phase.t}
                </span>
                <span className="flex flex-col leading-tight">
                  <span className="text-[13px] text-gray-500 dark:text-gray-400">현재 ONI</span>
                  <span className={"num text-[23px] font-bold " + phase.c}>{oni! > 0 ? "+" : ""}{oni!.toFixed(2)}</span>
                </span>
              </div>
              <p className="mt-2.5 text-[14px] leading-relaxed text-gray-700 dark:text-gray-200"><b className="font-semibold">가전 함의</b> {phase.d}</p>
              <div className="mt-2 flex gap-1 text-[11.5px] font-semibold">
          ...
          ) : <div className="flex h-40 items-center justify-center text-[14px] text-gray-400">불러오는 중</div>}
        </Panel>
        ...
        {enso.series.length ? (
          ...
          />
        ) : <Panel title="ONI 지수 추이" meaning="데이터 로딩" src="NOAA"><div className="flex h-40 items-center justify-center text-[14px] text-gray-400">불러오는 중</div></Panel>}
        ...
        {cdd.series.length ? (
          ...
          />
        ) : <Panel title="냉방도일 (CDD)" meaning="데이터 로딩" src="PAGASA"><div className="flex h-40 items-center justify-center text-[14px] text-gray-400">불러오는 중</div></Panel>}
        ...
        >
          <div className="flex flex-col divide-y divide-gray-50 dark:divide-gray-800/60">
            {tys.length === 0 ? <div className="flex h-32 items-center justify-center text-[14px] text-gray-400">최근 태풍 없음</div> : tys.slice(0, 6).map((t) => {
              const sig = SIG[t.maxSignal] ?? SIG[0]
              return (
                <div key={t.name + t.asOf} className="flex items-start gap-2.5 py-2">
                  <span className={"mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-[11.5px] font-bold " + sig.c}>{sig.t}</span>
                  <span className="min-w-0 flex-1">
                    <span className="flex items-baseline gap-1.5">
                      <b className="text-[15px] font-bold text-gray-900 dark:text-gray-50">{t.name}</b>
                      <span className="text-[12.5px] text-gray-500 dark:text-gray-400">{t.category}</span>
                      {t.inPar && <span className="rounded bg-rose-50 dark:bg-rose-500/10 px-1 text-[11px] font-bold text-rose-600 dark:text-rose-400">PAR</span>}
                    </span>
                    <span className="mt-0.5 line-clamp-1 block text-[13px] leading-snug text-gray-500 dark:text-gray-400">{t.headline}</span>
                  </span>
                  <span className="shrink-0 text-[12.5px] text-gray-400 dark:text-gray-500">{relDays(t.asOf)}</span>
                </div>
              )
            })}
          </div>
        ...
        >
          <div className="flex h-full flex-col">
            <div className="mb-1.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[13px] text-gray-500 dark:text-gray-400">
            ...
            <div className="h-[150px] w-full">{eqs.length ? <QuakeStrip quakes={eqs} /> : <div className="flex h-full items-center justify-center text-[14px] text-gray-400">불러오는 중</div>}</div>
      ...
      <p className="text-[13px] leading-relaxed text-gray-400 dark:text-gray-500">냉방도일(CDD)·기온=PAGASA 관측 기반 · 태풍=PAGASA 공보 · 지진=USGS FDSN 실시간(필리핀 4~21°N/116~128°E, M4.0+) · 화산 경보(PHIVOLCS)는 공식 API 부재로 추후 반영</p>
    </div>
  )
}
