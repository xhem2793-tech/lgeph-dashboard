  ...
  return (
    <div className="flex h-full flex-col rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3.5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md" style={{ animation: "fadeUp .5s cubic-bezier(.16,1,.3,1) both", animationDelay: delay + "s" }}>
      <div className="flex items-center gap-1.5">
        ...
        <span className="ml-auto text-[12.5px] font-medium text-gray-400 dark:text-gray-500">{unit}</span>
      </div>
      <div className="mt-1.5 flex min-h-[22px] items-center gap-x-3 text-[12.5px]">
        ...
        <span className="ml-auto flex items-baseline gap-1.5">
          ...
          {chg != null && <span className={"text-[13px] font-bold tabular-nums " + (up ? "text-rose-600 dark:text-rose-400" : "text-emerald-600 dark:text-emerald-400")}>{up ? "▲" : "▼"}{Math.abs(chg).toFixed(dec)} <span className="font-medium text-gray-400 dark:text-gray-500">30일</span></span>}
        </span>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateRows: aiOpen ? "1fr" : "0fr", transition: "grid-template-rows .3s cubic-bezier(.16,1,.3,1)" }}><div className="overflow-hidden"><div className="mt-1.5 border-l-2 border-indigo-300 dark:border-indigo-500/40 pl-2.5"><p className="text-[13px] leading-relaxed text-gray-600 dark:text-gray-300">{ai}</p></div></div></div>
      <p className="mt-auto border-t border-gray-100 dark:border-gray-800 pt-2 text-[12px] leading-relaxed text-gray-400 dark:text-gray-500"><b className="font-semibold text-gray-500 dark:text-gray-400">자료</b> {src}</p>
    </div>
  )
}
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl border border-indigo-100 dark:border-indigo-500/25 bg-gradient-to-r from-indigo-50 dark:from-indigo-500/10 via-indigo-50/40 dark:via-transparent to-white dark:to-gray-900 px-4 py-3 shadow-sm" style={{ animation: "fadeUp .5s ease both" }}>
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1 text-[15px] leading-snug text-gray-700 dark:text-gray-200"><b className="font-semibold text-gray-900 dark:text-gray-50">일일동향</b> — 통합 경제 대시보드 + 환율·유가 30일 · 조달원가·수요의 초단기 신호</div>
          <div className="hidden shrink-0 flex-wrap gap-1.5 sm:flex">{status.map((s) => <span key={s.t} className={"rounded-full px-2 py-0.5 text-[12.5px] font-bold " + cmap[s.c]}>{s.t}</span>)}</div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-sm" style={{ animation: "fadeUp .5s cubic-bezier(.16,1,.3,1) both", animationDelay: ".05s" }}>
        <div className="mb-3 flex items-center gap-2.5 border-b border-gray-100 dark:border-gray-800 pb-2.5">
          ...
          <h2 className="text-[18px] font-bold tracking-tight text-gray-900 dark:text-gray-50">통합 경제 지표</h2>
          <span className="text-[13px] font-semibold text-gray-400 dark:text-gray-500">핵심 지표 최신값 · 한눈에 보는 시장 상황</span>
        </div>
          ...
          {KPIS.map((k, i) => { const d = kv[k.key]; const t = d ? k.tone(d.value) : "flat"
            return (
              <div key={k.key} className="rounded-lg border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30 px-3 py-2.5" style={{ animation: "fadeUp .4s ease both", animationDelay: (0.08 + i * 0.03) + "s" }}>
                <div className="text-[12.5px] font-semibold text-gray-400 dark:text-gray-500">{k.label}</div>
                <div className={"mt-1 text-[22px] font-extrabold tabular-nums tracking-tight " + toneCls[t]}>{d ? (k.dec === 0 ? Math.round(d.value).toLocaleString() : d.value.toFixed(k.dec)) : "—"}<span className="ml-0.5 text-[13px] font-semibold text-gray-400 dark:text-gray-500">{k.unit}</span></div>
                <div className="mt-0.5 text-[11.5px] text-gray-400 dark:text-gray-500">{d ? d.date.slice(2, 4) + "." + Number(d.date.slice(5, 7)) + (d.date.slice(8) !== "01" ? "." + Number(d.date.slice(8, 10)) : "") : ""}</div>
              </div>
            )
          })}
        </div>
