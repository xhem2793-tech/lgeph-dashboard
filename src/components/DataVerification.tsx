  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="지표·출처 검색 (예: 물가, World Bank, 정책금리)"
        className="w-full max-w-md rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-3.5 py-2 text-[14px] text-gray-800 dark:text-gray-100 outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-500/20" />
        ...
        return (
          <section key={source} className="overflow-hidden rounded-xl" style={{ animation: "fadeUp .5s ease both" }}>
            <header className="flex flex-wrap items-center gap-2 border-b border-gray-100 dark:border-gray-800 px-4 py-2.5">
              ...
              <span className={"rounded px-1.5 py-0.5 text-[10.5px] font-bold " + t.cls}>{t.label}</span>
              <span className="text-[12px] text-gray-400 dark:text-gray-500">{items.length}개 지표</span>
              {sourceLink(source, items[0].source_ref) && (
                <a href={sourceLink(source, items[0].source_ref)!} target="_blank" rel="noopener noreferrer" className="ml-auto text-[12px] font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">원본 포털 ↗</a>
              )}
            </header>
                  ...
                  {items.map((r) => {
                    ...
                    return (
                      <tr key={r.indicator} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-indigo-50/40 dark:hover:bg-indigo-500/5">
                        ...
                        <td className="px-2 py-1.5 font-mono text-[11.5px] text-gray-500 dark:text-gray-400">{r.source_ref?.replace(/^https?:\/\/\S+/, "URL") ?? "—"}</td>
