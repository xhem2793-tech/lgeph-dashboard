...
export default function TodayBrief() {
  ...
  return (
    <section className="h-full rounded-xl border-[1.5px] border-indigo-500 bg-indigo-50/40 p-4">
      <header className="mb-2.5 flex items-baseline justify-between gap-2">
        <div className="flex items-baseline gap-2">
          <h2 className="text-[15px] font-bold tracking-tight text-gray-900">오늘의 핵심</h2>
          <span className="text-[10px] text-gray-400">{fmtDate(b?.asOf)}</span>
        </div>

        {b === undefined ? null : approved ? (
          <span className="rounded border border-emerald-200 bg-emerald-50 px-1.5 py-px text-[9px] font-bold text-emerald-700">
          ...
          <button
            ...
            onClick={onApprove}
            disabled={busy || !b}
            className="rounded border border-amber-300 bg-amber-50 px-1.5 py-px text-[9px] font-bold text-amber-700 transition-colors duration-200 hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-700 disabled:opacity-50"
      ...
      {b === undefined ? (
        ...
        <>
          {b.lines.map((l, i) => (
            <div
              key={i}
              className="relative mb-2 pl-3 last:mb-0"
            ...
            >
              ...
              <p className="text-[11.5px] leading-relaxed text-gray-800">{l.text}</p>
              {l.evidence ? (
                <p className="mt-0.5 text-[9px] text-gray-400">근거 · {l.evidence}</p>
              ) : null}
            </div>
          ))}

          {b.weeklyCall ? (
            <p className="mt-3 border-t border-indigo-200/60 pt-2.5 text-[10px] leading-relaxed text-gray-500">
              이번 주 판단 ·{" "}
          ...
          {err ? <p className="mt-1 text-[9.5px] text-rose-600">{err}</p> : null}
