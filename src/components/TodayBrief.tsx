    ...
    <section className="flex h-full flex-col rounded-xl border-[1.5px] border-indigo-500 bg-indigo-50/40 dark:bg-indigo-500/10 p-3.5 transition-all duration-300 ease-out hover:-translate-y-0.5 hover:shadow-lg hover:shadow-indigo-100">
      <header className="mb-1 flex items-baseline justify-between gap-2">
        <div className="flex items-baseline gap-2">
          <h2 className="text-[17px] font-bold tracking-tight text-gray-900 dark:text-gray-50">{t("brief_title")}</h2>
        </div>

        <div className="flex items-baseline gap-2">
          <span className="text-[11px] text-gray-400 dark:text-gray-500">{t("news_updated")} {stamp ? fmtStamp(stamp, lang === "en") : fmtDate(b?.asOf)}</span>
          {b === undefined ? null : approved ? (
            <span className="rounded border border-emerald-200 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-px text-[11px] font-bold text-emerald-700 dark:text-emerald-300">
            ...
            <button
              ...
              onClick={onApprove}
              disabled={busy || !b}
              className="rounded border border-amber-300 dark:border-amber-500/40 bg-amber-50 dark:bg-amber-500/10 px-1.5 py-px text-[11px] font-bold text-amber-700 dark:text-amber-300 transition-colors duration-200 hover:border-emerald-300 dark:hover:border-emerald-500/40 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 hover:text-emerald-700 dark:hover:text-emerald-300 disabled:opacity-50"
      ...
      {b === undefined ? (
        ...
        <p className="py-6 text-center text-[13px] text-gray-400 dark:text-gray-500">
          {t("brief_empty")}
        </p>
      ) : (
        <>
            ...
            {(lang === "en" && b.linesEn && b.linesEn.length ? b.linesEn : b.lines).map((l, i) => (
              ...
              >
                <div className="flex items-start gap-2">
                  <span className="mt-[2px] flex h-[16px] w-[16px] shrink-0 items-center justify-center rounded-full bg-indigo-500 text-[10px] font-bold text-white">
                    {i + 1}
                  </span>
                  <div className="min-w-0">
                    ...
                    {l.evidence ? (
                      <p className="mt-0.5 text-[11px] leading-4 text-gray-400 dark:text-gray-500">{t("brief_evidence")} · {l.evidence}</p>
                    ) : null}
                  </div>
                </div>
              </div>
          ...
          {err ? <p className="mt-1 text-[11px] text-rose-600 dark:text-rose-400">{t("brief_fail")}</p> : null}
