...
export function InsightBanner({ banner, open, onToggle }: { banner: Banner; open: boolean; onToggle: () => void }) {
  const { pick } = useLang()
  const period = pick(banner.period, banner.period_en)
  return (
    <div
      onClick={onToggle}
    ...
    >
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="min-w-0 flex-1 truncate text-[12.5px] text-gray-700 dark:text-gray-200">
          <b className="font-semibold text-gray-900 dark:text-gray-50">{pick(banner.title, banner.title_en)}</b> — {pick(banner.summary, banner.summary_en)}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateRows: open ? "1fr" : "0fr", transition: "grid-template-rows .36s cubic-bezier(.22,1,.36,1)" }}>
        <div className="overflow-hidden">
          <div className="border-t border-indigo-100/70 dark:border-indigo-500/25 px-4 pb-3.5 pt-3">
            <p className="text-[12.5px] leading-relaxed text-gray-700 dark:text-gray-200">{rich(pick(banner.body, banner.body_en))}</p>
            <p className="mt-2 flex items-start gap-1.5 text-[12px] leading-relaxed text-indigo-700 dark:text-indigo-300">
              <span className="mt-0.5 shrink-0 rounded-md bg-indigo-600/10 dark:bg-indigo-500/20 px-1.5 py-0.5 text-[10px] font-bold text-indigo-700 dark:text-indigo-300">{pick(banner.insightLabel, banner.insightLabel_en) || T("LG 관점", "LG View")}</span>
              <span>{rich(pick(banner.insight, banner.insight_en))}</span>
            </p>
          </div>
        </div>
      </div>
