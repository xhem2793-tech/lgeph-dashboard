        ...
        <section className="min-w-0 rounded-xl p-4" style={{ animation: "fadeUp .34s cubic-bezier(.22,1,.36,1) both" }}>
          ...
          {!loaded ? (
            ...
            <div className="flex h-52 flex-col items-center justify-center gap-1 rounded-xl border border-dashed border-gray-200 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-900/60 text-center">
              <div className="text-[13.5px] font-bold text-gray-500 dark:text-gray-400">데이터 적재 대기</div>
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        {ra && (
          <div className="rounded-lg border border-gray-100 dark:border-gray-800 p-3">
            ...
            <p className="text-[21.5px] font-extrabold tabular-nums text-gray-900 dark:text-gray-50">${ra.lo.toFixed(1)}~{ra.hi.toFixed(1)}<span className="text-[12px] font-semibold text-gray-400">B</span> <span className="text-[12px] font-medium text-gray-400">중앙값 ${ra.med.toFixed(1)}B</span></p>
        ...
        {re && (
          <div className="rounded-lg border border-gray-100 dark:border-gray-800 p-3">
            ...
            <p className="text-[21.5px] font-extrabold tabular-nums text-gray-900 dark:text-gray-50">${re.lo.toFixed(1)}~{re.hi.toFixed(1)}<span className="text-[12px] font-semibold text-gray-400">B</span> <span className="text-[12px] font-medium text-gray-400">중앙값 ${re.med.toFixed(1)}B</span></p>
...
export function ApplianceView() {
  ...
  return (
    <Shell title="가전 선행지표" sub="생산자물가·수입액·가전물가·전기료 — 원가·공급 선행" win={win} setWin={setWin} loaded={loaded} empty={empty} d={d} accent="indigo"
...
export function RatesView() {
  ...
  return (
    <Shell title="통화·금리·신용" sub="기준금리·통화량 M3·가계신용 — 할부·카드 구매력" win={win} setWin={setWin} loaded={loaded} empty={empty} d={d} accent="indigo"
...
export function LaborView() {
  ...
  return (
    <Shell title="고용·임금·소득" sub="실업·경제활동참가·OFW 송금 — 가전 구매력" win={win} setWin={setWin} loaded={loaded} empty={empty} d={d} accent="indigo"
...
export function SentimentView() {
  ...
  return (
    <Shell title="기업·소비 심리" sub="소비자심리 CCI·기업심리 BCI·BES 기업경기 — 수요 선행" win={win} setWin={setWin} loaded={loaded} empty={empty} d={d} accent="indigo"
...
export function PricesView() {
  ...
  return (
    <Shell title="물가" sub="생활물가·에너지·주거/내구재 CPI 상승률 — 실질 구매력·원가" win={win} setWin={setWin} loaded={loaded} empty={empty} d={d} accent="indigo"
