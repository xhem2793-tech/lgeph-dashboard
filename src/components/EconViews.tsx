  ...
  return (
    <div className="rounded-xl p-4" style={{ animation: "fadeUp .34s cubic-bezier(.22,1,.36,1) both", animationDelay: "80ms" }}>
      <header className="flex items-baseline justify-between border-b border-gray-100 dark:border-gray-800 pb-2.5">
        <h2 className="text-[14.5px] font-bold tracking-tight text-gray-900 dark:text-gray-50">{T("예정 일정", "Schedule")}</h2>
        <span className="text-[11px] text-gray-400 dark:text-gray-500">{T("2주간", "2 wks")}</span>
      </header>
      ...
      <div className="grid items-start gap-4">
        <section className="min-w-0" style={{ animation: "fadeUp .34s cubic-bezier(.22,1,.36,1) both" }}>
          ...
          {!loaded ? (
            ...
            <div key={activeSub} className="grid items-stretch gap-4 sm:grid-cols-2 xl:grid-cols-3" style={{ animation: "fadeUp .35s cubic-bezier(.22,1,.36,1) both" }}>{sections ? curSub?.node : children}</div>
          )}
        </section>
      </div>
...
const buildOwnItems = (): { key: string; name: string }[] => [
  { key: "appl_own_cool", name: T("냉방·선풍기", "Cool·Fans") }, { key: "appl_own_tv", name: "TV" },
  { key: "appl_own_ref", name: T("냉장고", "Fridge") }, { key: "appl_own_wash", name: T("세탁기", "Washer") }, { key: "appl_own_mobile", name: T("휴대폰", "Mobile") },
]
function OwnershipCard({ d }: { d: Mon }) {
  ...
  return (
    <div className="relative z-0 flex h-full flex-col rounded-xl p-3.5 transition-all duration-300 ease-out hover:-translate-y-0.5 hover:shadow-md" style={{ animation: "fadeUp .34s cubic-bezier(.22,1,.36,1) both" }}>
      <div className="flex items-center gap-1.5">
        <h3 className="text-[13.5px] font-bold tracking-tight text-gray-900 dark:text-gray-50">{T("가전 보유율 (침투율)", "Appliance Ownership")}</h3>
        ...
        <span className="ml-auto shrink-0 text-[10.5px] font-medium text-gray-400 dark:text-gray-500">{T("가구 % · 2020", "% HH · 2020")}</span>
      </div>
      <div className="mt-3 flex flex-1 flex-col justify-center gap-2.5">
        {rows.map((r, i) => (
          <div key={r.key} className="flex items-center gap-2">
...
export function ApplianceView({ win = "5Y" }: { win?: string }) {
  ...
  const elec = build(d, n, [{ key: "meralco_residential_rate", name: T("가정용 전기료", "Res. elec."), color: C.ind, w: 2 }])
  const cdd = build(d, n, [{ key: "cdd_monthly", name: T("냉방도일 CDD", "Cooling days"), color: C.rose, w: 2 }]) // 에어컨 수요 선행
  const energy = build(d, n, [{ key: "energy_households", name: T("가정용 에너지소비", "HH energy use"), color: C.ind, w: 2 }]) // ktoe, 연간
  ...
  return (
    <Shell title={T("가전 선행지표", "Appliance Leading Indicators")} sub={T("생산자물가·수입액·가전물가·전기료 — 원가·공급 선행", "PPI · Imports · Appliance CPI · Electricity — cost & supply leading")} win={win} setWin={() => {}} hideWin loaded={loaded} empty={empty} d={d} accent="indigo"
      ...
      {show(["전 제품"]) && ppi.series.length > 0 && (
        <ChartCard seg={T("전 제품·CE·B2B", "All·CE·B2B")} title={T("가전 생산자물가 PPI", "Appliance PPI")} unit={T("전년비 %", "YoY %")} labels={ppi.labels} series={ppi.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("가전 PPI", "Appl. PPI")} b /><Lg c={C.rose} t={T("전기기기", "Elec. equip.")} /><Lg c={C.blue} t={T("전자", "Electronics")} /><Lg c={C.brown} t={T("제조업 전체", "Total mfg")} /></>}
      ...
      {show(["전 제품"]) && imp.series.length > 0 && (
        <ChartCard seg={T("전 제품·CE·B2B", "All·CE·B2B")} title={T("가전·전자·통신 수입액", "Appl·CE·Tel. Imports")} unit={T("백만$ · 연간", "US$M · Annual")} labels={imp.labels} series={imp.series} decimals={0} seriesUnit={T("백만$", "US$M")}
          legend={<><Lg c={C.ind} t={T("가전", "Appliances")} b /><Lg c={C.rose} t={T("소비자전자", "Consumer elec.")} /><Lg c={C.blue} t={T("통신기기", "Telecom")} /></>}
      ...
      {show(["전 제품", "에어컨(RAC)"]) && inf.series.length > 0 && (
        <ChartCard seg={T("가전·에어컨", "Appl.·AC")} title={T("가전 소비자물가 상승률", "Appliance CPI Growth")} unit={T("전년비 %", "YoY %")} labels={inf.labels} series={inf.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("가전 물가", "Appl. CPI")} b /><Lg c={C.rose} t={T("에어컨", "Aircon")} /><Lg c={C.brown} t={T("전체 CPI", "Headline")} /></>}
      ...
      {show(["전 제품", "에어컨(RAC)", "냉장고", "공조(B2B)"]) && elec.series.length > 0 && (
        <ChartCard seg={T("에어컨·냉장고·공조", "AC·Ref·HVAC")} title={T("가정용 전기요금 (Meralco)", "Res. Elec. Rate (Meralco)")} unit="₱/kWh" labels={elec.labels} series={elec.series} decimals={2}
          legend={<Lg c={C.ind} t={T("가정용 전기료", "Res. elec.")} b />}
      ...
      {show(["에어컨(RAC)"]) && cdd.series.length > 0 && (
        <ChartCard seg={T("에어컨", "AC")} title={T("냉방도일 (에어컨 수요 선행)", "Cooling Degree Days (CDD)")} unit={T("CDD · 월별 · 기준24℃", "CDD · Monthly · 24°C")} labels={cdd.labels} series={cdd.series} decimals={0} seriesUnit="CDD"
          legend={<Lg c={C.rose} t={T("냉방도일 CDD", "Cooling days")} b />}
      ...
      {show(["전 제품"]) && energy.series.length > 0 && (
        <ChartCard seg={T("전 제품", "All")} title={T("가정용 에너지소비", "HH Energy Use")} unit={T("ktoe · 연간", "ktoe · Annual")} kind="bar" labels={energy.labels} series={energy.series} decimals={0} seriesUnit="ktoe"
          legend={<Lg c={C.ind} t={T("가정용 에너지소비", "HH energy use")} b />}
      ...
      {show(["전 제품"]) && elecpc.series.length > 0 && (
        <ChartCard seg={T("전 제품", "All")} title={T("1인당 전력소비", "Elec. per Cap.")} unit={T("kWh/인 · 연간", "kWh/person · Annual")} labels={elecpc.labels} series={elecpc.series} decimals={0} seriesUnit="kWh"
          legend={<Lg c={C.ind} t={T("1인당 전력소비", "Elec. per cap.")} b />}
      ...
      {show(["전 제품", "냉장고", "세탁기", "에어컨(RAC)", "TV"]) && (
        <ChartCard seg={T("전 제품·CE", "All·CE")} title={T("품목별 가전 수입 규모 (HS)", "Appliance Imports by HS")} unit={T("백만$ · 연간", "US$M · Annual")} labels={HS_YRS} series={[{ name: T("에어컨", "Aircon"), color: C.rose, w: 2, data: HS_TOTALS.ac }, { name: T("냉장고", "Fridge"), color: C.ind, w: 2, data: HS_TOTALS.ref }, { name: T("TV·모니터", "TV·Monitor"), color: C.blue, data: HS_TOTALS.tv }, { name: T("세탁기", "Washer"), color: C.emer, data: HS_TOTALS.wash }]} decimals={0} seriesUnit={T("백만$", "US$M")}
          legend={<><Lg c={C.rose} t={T("에어컨", "Aircon")} b /><Lg c={C.ind} t={T("냉장고", "Fridge")} b /><Lg c={C.blue} t={T("TV·모니터", "TV·Monitor")} /><Lg c={C.emer} t={T("세탁기", "Washer")} /></>}
...
export function RatesView() {
  ...
  const n = WIN.find((w) => w.k === win)!.n
  const pol = build(d, n, [{ key: "policy_rate_monthly", name: T("정책금리 RRP", "Policy (RRP)"), color: C.ind, w: 2 }, { key: "interbank_call_rate", name: T("시장금리(콜)", "Market (call)"), color: C.rose }]) // 정책금리+시장금리 월별
  const loan = build(d, n, [{ key: "consumer_loan_growth_yoy", name: T("소비자대출", "Consumer loans"), color: C.ind, w: 2 }, { key: "bank_loan_growth_yoy", name: T("은행 총대출", "Bank loans"), color: C.blue }])
  const m3 = build(d, n, [{ key: "m3_growth_yoy", name: T("광의통화(M3)", "Broad money"), color: C.ind, w: 2 }]) // IMF IFS 월별 YoY
  const credit = build(d, n, [{ key: "domestic_credit_pct_gdp", name: T("민간신용(%GDP)", "Priv. credit (%GDP)"), color: C.ind, w: 2 }])
  const cab = build(d, n, [{ key: "current_account_pct_gdp", name: T("경상수지(%GDP)", "Curr. acct (%GDP)"), color: C.ind, w: 2 }]) // 대외균형, 연간
  const fdi = build(d, n, [{ key: "fdi_net_inflow_usd", name: T("FDI 순유입", "FDI inflow"), color: C.emer, w: 2, tf: (v) => v / 1e9 }]) // USD→십억$, 연간
  const trade = build(d, n, [{ key: "exports_gdp", name: T("수출", "Exports"), color: C.emer, w: 2 }, { key: "imports_gdp", name: T("수입", "Imports"), color: C.rose }, { key: "trade_balance_gdp", name: T("무역수지", "Trade bal."), color: C.ind, w: 2 }]) // %GDP
  const reserves = build(d, n, [{ key: "reserves_usd", name: T("외환보유액", "FX reserves"), color: C.ind, w: 2, tf: (v) => v / 1e9 }]) // USD→십억$
  const govt = build(d, n, [{ key: "govt_exp_gdp", name: T("정부지출", "Govt exp."), color: C.ind, w: 2 }, { key: "services_pct_gdp", name: T("서비스업 비중", "Services share"), color: C.emer }]) // %GDP
  const cards = build(d, n, [{ key: "credit_card_ownership", name: T("신용카드 보유", "Credit card"), color: C.ind, w: 2 }, { key: "debit_card_ownership", name: T("직불카드 보유", "Debit card"), color: C.blue }, { key: "account_ownership", name: T("계좌 보유", "Account"), color: C.emer }]) // Findex 연간 %
  const finuse = build(d, n, [{ key: "borrowed_any_pct", name: T("차입 경험", "Borrowed"), color: C.rose, w: 2 }, { key: "saved_at_fi_pct", name: T("금융기관 저축", "Saved at FI"), color: C.emer }]) // Findex 연간 %
  ...
  return (
    <Shell title={T("통화·금리·신용", "Money, Rates & Credit")} sub={T("기준금리·통화량 M3·가계신용 — 할부·카드 구매력", "Policy rate · M3 · Household credit — installment/card purchasing power")} win={win} setWin={setWin} loaded={loaded} empty={empty} d={d} accent="indigo"
      ...
      kpiDefs={[
        { key: "BSP_policy_rate", label: T("정책금리 RRP", "Policy (RRP)"), fmt: (v) => v + "%", tone: "amber" },
      ...
      {pol.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("정책금리·시장금리", "Policy vs Market")} unit={T("% · 월별", "% · Monthly")} labels={pol.labels} series={pol.series} decimals={2} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("정책금리 RRP", "Policy (RRP)")} b /><Lg c={C.rose} t={T("시장금리(콜)", "Market (call)")} /></>}
      ...
      {loan.series.length > 0 && (
        <ChartCard seg="CE" title={T("가계·기업 대출 증가율", "HH·Corp Loan Growth")} unit={T("전년비 % · 월별", "YoY % · Monthly")} labels={loan.labels} series={loan.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("소비자대출", "Consumer loans")} b /><Lg c={C.blue} t={T("은행 총대출", "Bank loans")} /></>}
      ...
      {credit.series.length > 0 && (
        <ChartCard seg="CE" title={T("민간신용 침투 (% GDP)", "Priv. Credit (%GDP)")} unit={T("% GDP · 연간", "% GDP · Annual")} labels={credit.labels} series={credit.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.ind} t={T("민간신용(%GDP)", "Priv. credit (%GDP)")} b />}
        ...
        { key: "consumer_fin", label: T("소비 금융·결제", "Finance·Payments"), node: <>
      {cards.series.length > 0 && (
        <ChartCard seg="CE" title={T("카드·계좌 보급률", "Card·Acct Ownership")} unit={T("% 성인 · 연간", "% of adults · Annual")} labels={cards.labels} series={cards.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("신용카드 보유", "Credit card")} b /><Lg c={C.blue} t={T("직불카드 보유", "Debit card")} /><Lg c={C.emer} t={T("계좌 보유", "Account")} /></>}
      ...
      {finuse.series.length > 0 && (
        <ChartCard seg="CE" title={T("차입·저축 행태", "Borrowing·Saving")} unit={T("% 성인 · 연간", "% of adults · Annual")} labels={finuse.labels} series={finuse.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.rose} t={T("차입 경험", "Borrowed")} b /><Lg c={C.emer} t={T("금융기관 저축", "Saved at FI")} /></>}
        ...
        { key: "fiscal_cap", label: T("재정·자본시장", "Fiscal·Markets"), node: <>
      ...
      {psei.series.length > 0 && (
        <ChartCard seg="B2B" title={T("PSEi 주가지수 (월말)", "PSEi Index (Month-end)")} unit={T("지수 · 월별", "Index · Monthly")} labels={psei.labels} series={psei.series} decimals={0} seriesUnit=""
          legend={<Lg c={C.ind} t={T("PSEi 종합지수", "PSEi composite")} b />}
      ...
      {npl.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("은행 부실채권(NPL) 비율", "Bank NPL Ratio")} unit={T("% · 연간", "% · Annual")} labels={npl.labels} series={npl.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.rose} t={T("NPL 비율(총대출 대비)", "NPL ratio (of loans)")} b />}
        ...
        { key: "money_ext", label: T("통화·대외", "Money·Ext"), node: <>
      {m3.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("통화량 M3 증가율", "M3 Money Growth")} unit={T("전년비 % · 월별", "YoY % · Monthly")} labels={m3.labels} series={m3.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.ind} t={T("광의통화(M3)", "Broad money")} b />}
      ...
      {cab.series.length > 0 && (
        <ChartCard seg="B2B" title={T("경상수지 (% GDP)", "Curr. Acct (%GDP)")} unit={T("% GDP · 연간", "% GDP · Annual")} kind="bar" labels={cab.labels} series={cab.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.ind} t={T("경상수지(%GDP)", "Curr. acct (%GDP)")} b />}
      ...
      {fdi.series.length > 0 && (
        <ChartCard seg="B2B" title={T("외국인직접투자 순유입 (FDI)", "Net FDI Inflow")} unit={T("십억$ · 연간", "US$B · Annual")} kind="bar" labels={fdi.labels} series={fdi.series} decimals={1} seriesUnit={T("십억$", "US$B")}
          legend={<Lg c={C.emer} t={T("FDI 순유입", "FDI inflow")} b />}
      ...
      {trade.series.length > 0 && (
        <ChartCard seg="B2B" title={T("대외거래 (수출·수입·무역수지)", "Ext. Trade (Exp·Imp·Bal)")} unit={T("% GDP · 연간", "% GDP · Annual")} labels={trade.labels} series={trade.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.emer} t={T("수출", "Exports")} b /><Lg c={C.rose} t={T("수입", "Imports")} /><Lg c={C.ind} t={T("무역수지", "Trade bal.")} /></>}
      ...
      {govt.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("정부지출·서비스업 비중", "Govt·Services Share")} unit={T("% GDP · 연간", "% GDP · Annual")} labels={govt.labels} series={govt.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("정부지출", "Govt exp.")} b /><Lg c={C.emer} t={T("서비스업 비중", "Services share")} /></>}
...
export function GrowthView() {
  ...
  const cons = build(d, n, [{ key: "construction_gva_growth", name: T("건설 부가가치", "Const. GVA"), color: C.ind, w: 2 }, { key: "construction_gfcf_growth", name: T("건설 투자", "Const. invest."), color: C.violet }])
  const ind = build(d, n, [{ key: "industry_gva_yoy", name: T("산업", "Industry"), color: C.ind, w: 2 }, { key: "manufacturing_va_growth", name: T("제조업", "Mfg"), color: C.rose }])
  const cap = build(d, n, [{ key: "capacity_utilization", name: T("평균 가동률", "Avg utilization"), color: C.amber, w: 2 }]) // 레벨(%) — 성장률과 축 분리
  const ret = build(d, n, [{ key: "wholesale_retail_trade_yoy", name: T("도소매 거래", "W&R trade"), color: C.ind, w: 2 }, { key: "retail_gva_growth", name: T("소매 부가가치", "Retail GVA"), color: C.teal }, { key: "wholesale_gva_growth", name: T("도매 부가가치", "Wholesale GVA"), color: C.amber }])
  const permit = build(d, n, [{ key: "permits_nonresidential_floorarea", name: T("비주거 착공면적", "Non-res. floor area"), color: C.ind, w: 2, tf: (v) => v / 1e6 }])
  const permitV = build(d, n, [{ key: "permits_residential_value", name: T("주거 건축허가액", "Res. permit value"), color: C.violet, w: 2, tf: (v) => v / 1e6 }]) // 천PHP→십억₱
  const va = build(d, n, [{ key: "manufacturing_va_growth", name: T("제조업", "Mfg"), color: C.ind, w: 2 }, { key: "industry_gva_yoy", name: T("산업", "Industry"), color: C.rose }, { key: "services_gva_yoy", name: T("서비스", "Services"), color: C.emer }]) // 산업·서비스는 최신 vintage(gva_yoy)로 통일 — 구 va_growth와 값 불일치 제거
  const rsale = build(d, n, [{ key: "retail_sales_growth", name: T("소매판매 증가율", "Retail sales growth"), color: C.ind, w: 2 }]) // 연간 6년(COVID 저점)
  const pcap = build(d, n, [{ key: "gdp_per_capita_usd", name: T("1인당 GDP", "GDP per cap."), color: C.ind, w: 2 }]) // USD, 연간 — 구매력·시장규모
  const tour = build(d, n, [{ key: "tourism_arrivals", name: T("국제 관광객", "Int'l tourists"), color: C.teal, w: 2 }]) // 백만명, 연간 — 서비스·소비 동력
  const office = build(d, n, [{ key: "office_vacancy_ncr", name: T("오피스 공실률", "Office vacancy"), color: C.rose, w: 2 }]) // 민간자료(Colliers)
    ...
    <Shell title={T("국민계정·성장", "National Accounts & Growth")} sub={T("GDP·소비·투자·건설허가·산업·유통 — 가전 수요 파이", "GDP · Consumption · Investment · Permits · Industry · Trade — the appliance demand pie")} win={win} setWin={setWin} loaded={loaded} empty={empty} d={d}
      ...
      {pcap.series.length > 0 && (
        <ChartCard seg="CE" title={T("1인당 GDP", "GDP per Cap.")} unit={T("US$ · 연간", "US$ · Annual")} kind="bar" labels={pcap.labels} series={pcap.series} decimals={0} seriesUnit="$"
          legend={<Lg c={C.ind} t={T("1인당 GDP", "GDP per cap.")} b />}
      ...
      {tour.series.length > 0 && (
        <ChartCard seg="CE" title={T("국제 관광객 입국자수", "Int'l Tourist Arrivals")} unit={T("백만명 · 연간", "Million · Annual")} kind="bar" labels={tour.labels} series={tour.series} decimals={1} seriesUnit="M"
          legend={<Lg c={C.teal} t={T("국제 관광객(백만명)", "Int'l tourists (M)")} b />}
      ...
      {ind.series.length > 0 && (
        <ChartCard seg="B2B" title={T("산업·제조 성장률", "Industry·Mfg Growth")} unit={T("전년비 %", "YoY %")} labels={ind.labels} series={ind.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("산업", "Industry")} b /><Lg c={C.rose} t={T("제조업", "Mfg")} /></>}
      ...
      {cap.series.length > 0 && (
        <ChartCard seg="B2B" title={T("평균 가동률", "Avg Utilization")} unit={T("% (레벨)", "% (level)")} labels={cap.labels} series={cap.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.amber} t={T("평균 가동률", "Avg utilization")} b />}
      ...
      {va.series.length > 0 && (
        <ChartCard seg="B2B" title={T("부문별 부가가치 성장", "Sector GVA Growth")} unit={T("전년비 %", "YoY %")} labels={va.labels} series={va.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("제조업", "Mfg")} b /><Lg c={C.rose} t={T("산업", "Industry")} /><Lg c={C.emer} t={T("서비스", "Services")} /></>}
        ...
        { key: "construction", label: T("건설·투자", "Const.·Invest."), node: <>
      {cons.series.length > 0 && (
        <ChartCard seg="B2B" title={T("건설 부가가치·투자 성장", "Construction GVA·Invest")} unit={T("전년비 %", "YoY %")} labels={cons.labels} series={cons.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("건설 부가가치", "Const. GVA")} b /><Lg c={C.violet} t={T("건설 투자", "Const. invest.")} /></>}
      ...
      {permitV.series.length > 0 && (
        <ChartCard seg="B2B" title={T("주거 건축허가액", "Res. Permit Value")} unit={T("십억₱ · 분기", "₱B · Quarterly")} kind="bar" labels={permitV.labels} series={permitV.series} decimals={1} seriesUnit={T("십억₱", "₱B")}
          legend={<Lg c={C.violet} t={T("주거 건축허가액", "Res. permit value")} b />}
      ...
      {permit.series.length > 0 && (
        <ChartCard seg="B2B" title={T("비주거 건축허가(상업·산업)", "Non-Res. Permits (Comm·Ind)")} unit={T("백만 ㎡ · 분기", "Million ㎡ · Qtr")} kind="bar" labels={permit.labels} series={permit.series} decimals={2}
          legend={<Lg c={C.ind} t={T("비주거 착공면적", "Non-res. floor area")} b />}
      ...
      {office.series.length > 0 && (
        <ChartCard seg="B2B" title={T("오피스 공실률 (메트로 마닐라)", "Office Vacancy (Metro Manila)")} unit={T("% · 분기 · 민간자료", "% · Quarterly · Private data")} labels={office.labels} series={office.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.rose} t={T("오피스 공실률", "Office vacancy")} b />}
      ...
      {rrepi.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("주거용 부동산가격 상승률", "Res. Property Price Growth")} unit={T("전년비 % · 분기", "YoY % · Quarterly")} labels={rrepi.labels} series={rrepi.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("명목", "Nominal")} b /><Lg c={C.teal} t={T("실질(물가조정)", "Real (infl-adj.)")} /></>}
      ...
      {ret.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("도소매 유통 성장", "W&R Trade Growth")} unit={T("전년비 %", "YoY %")} labels={ret.labels} series={ret.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("도소매 거래", "W&R trade")} b /><Lg c={C.teal} t={T("소매 부가가치", "Retail GVA")} /><Lg c={C.amber} t={T("도매 부가가치", "Wholesale GVA")} /></>}
      ...
      {rsale.series.length > 0 && (
        ...
        { key: "sea", label: T("동남아 비교", "SEA Compare"), node: <>
      {seaPPP.series.length > 0 && (
        <ChartCard seg="CE" title={T("1인당 GDP (PPP) — 동남아 6개국", "GDP per Cap. (PPP) — 6 SEA")} unit={T("천 int$ · 연간", "'000 int$ · Annual")} labels={seaPPP.labels} series={seaPPP.series} decimals={1} seriesUnit="k"
      ...
      {seaNom.series.length > 0 && (
        <ChartCard seg="CE" title={T("1인당 GDP (명목) — 동남아 6개국", "GDP per Cap. (Nominal) — SEA-6")} unit={T("천 US$ · 연간", "'000 US$ · Annual")} labels={seaNom.labels} series={seaNom.series} decimals={1} seriesUnit="k"
          legend={<>{SEA_SPECS.map((s) => <Lg key={s.key} c={s.color} t={s.name} b={s.key === "Philippines"} />)}</>}
...
export function LaborView() {
  ...
  const n = WIN.find((w) => w.k === win)!.n
  const un = build(d, n, [{ key: "unemployment_rate", name: T("실업률", "Unemp. rate"), color: C.ind, w: 2 }, { key: "underemployment_rate", name: T("불완전고용", "Underemp."), color: C.rose }])
  const lf = build(d, n, [{ key: "labor_force_participation_rate", name: T("경제활동참가율", "LF participation"), color: C.ind, w: 2 }])
  const emp = build(d, n, [{ key: "employed_persons", name: T("취업자 수", "Employed"), color: C.emer, w: 2 }]) // 백만명, 월별 5년
  const rem = build(d, n, [{ key: "ofw_cash_remittance_growth_yoy", name: T("송금 증가율", "Remit. growth"), color: C.ind, w: 2 }])
  const remL = build(d, n, [{ key: "ofw_personal_remittance", name: T("개인송금(현금+현물)", "Pers. remit. (cash+kind)"), color: C.ind, w: 2 }, { key: "ofw_cash_remittance", name: T("현금송금", "Cash remit."), color: C.emer, w: 2 }])
  const remY = build(d, n, [{ key: "remittances_usd", name: T("연간 송금액", "Annual remit."), color: C.emer, w: 2, tf: (v) => v / 1e9 }]) // USD→십억$, 연간 장기(15년)
  const pop = build(d, n, [{ key: "population", name: T("인구", "Population"), color: C.ind, w: 2, tf: (v) => v / 1e6 }]) // 명→백만명, 연간
  const wage = build(d, n, [{ key: "min_wage_php", name: T("최저임금(일급)", "Min. wage (daily)"), color: C.ind, w: 2 }]) // PHP/일, 11년
  ...
  const infra = build(d, n, [{ key: "internet_penetration", name: T("인터넷", "Internet"), color: C.ind, w: 2 }, { key: "electrification_rate", name: T("전기", "Elec."), color: C.emer }]) // %, 보급률
  const pov = build(d, n, [{ key: "poverty_rate", name: T("빈곤율", "Poverty rate"), color: C.ind, w: 2 }]) // %, 소득계층
  const urban = build(d, n, [{ key: "urban_population_pct", name: T("도시화율", "Urbanization"), color: C.ind, w: 2 }]) // %, 연간
  const age = build(d, n, [{ key: "median_age", name: T("중위연령", "Median age"), color: C.ind, w: 2 }]) // 세, 연간
  const fam = build(d, n, [{ key: "household_size", name: T("평균 가구원수", "Avg HH size"), color: C.ind, w: 2 }, { key: "fertility_rate", name: T("합계출산율", "Fertility rate"), color: C.rose }]) // 명, 연간
  ...
  const cons = build(d, n, [{ key: "hh_consumption_pc", name: T("1인당 가계소비", "HH consump. per cap."), color: C.ind, w: 2 }]) // 불변$
  const gni = build(d, n, [{ key: "gni_per_capita", name: T("1인당 GNI", "GNI per cap."), color: C.ind, w: 2 }]) // 명목$
  ...
  return (
    <Shell title={T("고용·임금·소득", "Employment, Wages & Income")} sub={T("실업·경제활동참가·OFW 송금 — 가전 구매력", "Unemployment · Participation · OFW remittances — appliance purchasing power")} win={win} setWin={setWin} loaded={loaded} empty={empty} d={d} accent="indigo"
      ...
      kpiDefs={[
        { key: "unemployment_rate", label: T("실업률", "Unemp. rate"), fmt: (v) => v + "%", tone: "rose" },
        { key: "underemployment_rate", label: T("불완전고용", "Underemp."), fmt: (v) => v + "%", tone: "rose" },
      ...
      sections={[
        { key: "emp_rem", label: T("고용·송금", "Jobs·Remit."), node: <>
      {un.series.length > 0 && (
        <ChartCard seg="CE" title={T("실업·불완전고용률", "Unemp. & Underemp.")} unit="%" labels={un.labels} series={un.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("실업률", "Unemp. rate")} b /><Lg c={C.rose} t={T("불완전고용", "Underemp.")} /></>}
      ...
      {remL.series.length > 0 && (
        <ChartCard seg="CE" title={T("OFW 송금액 (개인·현금)", "OFW Remit. (Pers·Cash)")} unit={T("십억$ · 월별", "US$B · Monthly")} labels={remL.labels} series={remL.series}
          legend={<><Lg c={C.ind} t={T("개인송금(현금+현물)", "Pers. remit. (cash+kind)")} b /><Lg c={C.emer} t={T("현금송금", "Cash remit.")} b /></>}
      ...
      {rem.series.length > 0 && (
        <ChartCard seg="CE" title={T("OFW 송금 증가율", "OFW Remit. Growth")} unit={T("전년비 %", "YoY %")} labels={rem.labels} series={rem.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.ind} t={T("송금 증가율", "Remit. growth")} b />}
      ...
      {lf.series.length > 0 && (
        <ChartCard seg="CE" title={T("경제활동참가율", "LF Participation")} unit="%" labels={lf.labels} series={lf.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.ind} t={T("경제활동참가율", "LF participation")} b />}
      ...
      {emp.series.length > 0 && (
        <ChartCard seg="CE" title={T("취업자 수", "Employment")} unit={T("백만명 · 월별", "Million · Monthly")} labels={emp.labels} series={emp.series} decimals={1} seriesUnit={T("백만명", "Million")}
          legend={<Lg c={C.emer} t={T("취업자 수", "Employed")} b />}
      ...
      {empShare.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("산업별 고용비중", "Emp. by Sector")} unit={T("% · 연간", "% · Annual")} labels={empShare.labels} series={empShare.series} decimals={0} seriesUnit="%"
      ...
      {remY.series.length > 0 && (
        <ChartCard seg="CE" title={T("연간 해외송금액", "Annual Remit.")} unit={T("십억$ · 연간", "US$B · Annual")} kind="bar" labels={remY.labels} series={remY.series} decimals={1} seriesUnit={T("십억$", "US$B")}
          legend={<Lg c={C.emer} t={T("연간 송금액", "Annual remit.")} b />}
        ...
        { key: "income", label: T("임금·가구", "Wages·HH"), node: <>
      {pop.series.length > 0 && (
      ...
      {wage.series.length > 0 && (
        <ChartCard seg="CE" title={T("최저임금 (일급)", "Min. Wage (Daily)")} unit={T("₱/일 · 연간", "₱/day · Annual")} kind="bar" labels={wage.labels} series={wage.series} decimals={0} seriesUnit="₱"
          legend={<Lg c={C.ind} t={T("최저임금(일급)", "Min. wage (daily)")} b />}
      ...
      {hh.series.length > 0 && (
        <ChartCard seg="CE" title={T("가구 수", "Households")} unit={T("백만가구 · 연간", "Million HH · Annual")} labels={hh.labels} series={hh.series} decimals={1} seriesUnit={T("백만", "Million")}
        ...
        { key: "demo", label: T("인구·구조", "Demographics"), node: <>
      {infra.series.length > 0 && (
        <ChartCard seg="CE" title={T("인터넷·전기 보급률", "Internet·Elec. Access")} unit={T("% · 연간", "% · Annual")} labels={infra.labels} series={infra.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("인터넷", "Internet")} b /><Lg c={C.emer} t={T("전기", "Elec.")} /></>}
      ...
      {urban.series.length > 0 && (
        <ChartCard seg="CE" title={T("도시화율", "Urbanization")} unit={T("% · 연간", "% · Annual")} labels={urban.labels} series={urban.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.ind} t={T("도시화율", "Urbanization")} b />}
      ...
      {fam.series.length > 0 && (
        <ChartCard seg="CE" title={T("가구원수·합계출산율", "HH Size & Fertility")} unit={T("명 · 연간", "Persons · Annual")} labels={fam.labels} series={fam.series} decimals={2} seriesUnit={T("명", "persons")}
          legend={<><Lg c={C.ind} t={T("평균 가구원수", "Avg HH size")} b /><Lg c={C.rose} t={T("합계출산율", "Fertility rate")} /></>}
      ...
      <CrossBarCard d={d} seg="CE" unit={T("인구 % · 2020", "% pop. · 2020")} title={T("인구 연령구조", "Age Structure")} sort={false}
        items={[{ key: "pop_share_child", name: T("유소년 0-14", "Kids 0-14") }, { key: "pop_share_youth", name: T("청년 20-39", "Youth 20-39") }, { key: "pop_share_working", name: T("생산가능 15-64", "Working 15-64") }, { key: "pop_share_old", name: T("고령 65+", "Aged 65+") }]}
        ...
        { key: "adopt", label: T("소비·기술·금융", "Consump·Tech·Fin."), node: <>
      {ict.series.length > 0 && (
        <ChartCard seg="CE" title={T("ICT 보급 (모바일·인터넷)", "ICT (Mobile·Internet)")} unit={T("100명당 · % · 연간", "Per 100 · % · Annual")} labels={ict.labels} series={ict.series} decimals={0} seriesUnit=""
      ...
      {fin.series.length > 0 && (
        <ChartCard seg="CE" title={T("금융계좌 보유율 (금융포용)", "Account Ownership")} unit={T("% · 연간", "% · Annual")} labels={fin.labels} series={fin.series} decimals={0} seriesUnit="%"
      ...
      {cons.series.length > 0 && (
        <ChartCard seg="CE" title={T("1인당 가계소비", "HH Consump. per Cap.")} unit={T("불변 US$ · 연간", "Constant US$ · Annual")} kind="bar" labels={cons.labels} series={cons.series} decimals={0} seriesUnit="$"
          legend={<Lg c={C.ind} t={T("1인당 가계소비", "HH consump. per cap.")} b />}
      ...
      {gni.series.length > 0 && (
        <ChartCard seg="CE" title={T("1인당 GNI (국민총소득)", "GNI per Capita")} unit={T("US$ · 연간", "US$ · Annual")} kind="bar" labels={gni.labels} series={gni.series} decimals={0} seriesUnit="$"
          legend={<Lg c={C.ind} t={T("1인당 GNI", "GNI per cap.")} b />}
      ...
      {gini.series.length > 0 && (
        <ChartCard seg="CE" title={T("소득불평등 (지니계수)", "Inequality (Gini)")} unit={T("index · 연간", "Index · Annual")} labels={gini.labels} series={gini.series} decimals={1} seriesUnit=""
  ...
  return (
    <div className="flex h-full min-w-0 flex-col rounded-xl p-3.5" style={{ animation: "fadeUp .34s cubic-bezier(.22,1,.36,1) both" }}>
      <div className="flex items-center gap-1.5">
        ...
        <span className="ml-auto shrink-0 text-[10.5px] font-medium text-gray-400 dark:text-gray-500">{T("%응답 · 25Q4 · 복수", "% resp · 25Q4 · Multi")}</span>
      </div>
      ...
      <button type="button" onClick={() => setMore((v) => !v)} className="mt-1.5 self-start text-[10.5px] font-semibold text-violet-600 dark:text-violet-400 transition-colors hover:text-violet-700 dark:hover:text-violet-300">{more ? T("접기 ▲", "Less ▲") : `${T("더보기", "More")} (+${BES_CONSTRAINTS.length - 6}) ▼`}</button>
...
export function SentimentView() {
  ...
  const cci = build(d, n, [{ key: "consumer_confidence_index", name: T("현재 CCI", "Current CCI"), color: C.ind, w: 2 }, { key: "consumer_confidence_next12m", name: T("향후 12개월", "Next 12 mo."), color: C.emer }])
  const bci = build(d, n, [{ key: "business_confidence_index", name: T("현재 BCI", "Current BCI"), color: C.ind, w: 2 }, { key: "business_confidence_next12m", name: T("향후 12개월", "Next 12 mo."), color: C.blue }])
  const dur = build(d, n, [{ key: "durables_buying_intention", name: T("내구재 구매의향", "Durables intent"), color: C.ind, w: 2 }])
  // BES(기업경기전망조사) 분기 — 2001~2025
  const besOverall = build(d, n, [{ key: "bes_overall_ci", name: T("당분기", "Current Q"), color: C.ind, w: 2 }, { key: "bes_ci_next_q", name: T("차분기 전망", "Next-Q outlook"), color: C.emer }])
  const besSector = build(d, n, [{ key: "bes_ci_manufacturing", name: T("제조업", "Mfg"), color: C.rose, w: 2 }, { key: "bes_ci_retail", name: T("도소매", "Trade"), color: C.ind }, { key: "bes_ci_services", name: T("서비스", "Services"), color: C.emer }])
  const besOps = build(d, n, [{ key: "bes_credit_access", name: T("신용접근", "Credit access"), color: C.ind, w: 2 }, { key: "bes_financial_condition", name: T("재무여건", "Fin. condition"), color: C.rose }])
  const besJob = build(d, n, [{ key: "bes_employment_outlook", name: T("고용전망(차분기)", "Emp. outlook (next Q)"), color: C.ind, w: 2 }, { key: "bes_expansion_plans", name: T("설비확장 계획", "Expansion plans"), color: C.emer }])
  const besCap = build(d, n, [{ key: "bes_capacity_util", name: T("평균 가동률", "Avg utilization"), color: C.amber, w: 2 }])
  const besCon = build(d, n, [{ key: "bes_con_competition", name: T("경쟁 심화", "Competition"), color: C.rose, w: 2.4 }, { key: "bes_con_demand", name: T("수요 부족", "Weak demand"), color: C.ind, w: 2 }, { key: "bes_con_interest", name: T("고금리", "High rates"), color: C.amber }, { key: "bes_con_financial", name: T("재무 문제", "Financial problems"), color: C.emer }])
  const cesDur = build(d, n, [{ key: "ces_durables_buy", name: T("내구재 구매 가구%", "Durables-buying HH %"), color: C.emer, w: 2 }])
  ...
  return (
    <Shell title={T("기업·소비 심리", "Business & Consumer Sentiment")} sub={T("소비자심리 CCI·기업심리 BCI·BES 기업경기 — 수요 선행", "Consumer CCI · Business BCI · BES business cycle — demand leading")} win={win} setWin={setWin} loaded={loaded} empty={empty} d={d} accent="indigo"
      ...
      kpiDefs={[
        ...
        { key: "durables_buying_intention", label: T("내구재 구매의향", "Durables intent"), fmt: (v) => String(v), tone: "emerald" },
      ]}
      sections={[
        { key: "summary", label: T("심리 요약", "Sentiment"), node: <>
      {esi.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("경제심리지수 (자체산출·실험적)", "Economic Sentiment (In-house)")} unit={T("지수 · 평균100 · 분기", "Index · Mean 100 · Qtr")} labels={esi.labels} series={esi.series} decimals={1}
      ...
      {dur.series.length > 0 && (
        <ChartCard seg="CE" title={T("내구재 구매의향", "Durables Intent")} unit={T("지수", "Index")} labels={dur.labels} series={dur.series} decimals={1}
          legend={<Lg c={C.ind} t={T("내구재 구매의향", "Durables intent")} b />}
      ...
      {cci.series.length > 0 && (
        <ChartCard seg="CE" title={T("소비자심리 CCI", "Consumer CCI")} unit={T("지수", "Index")} labels={cci.labels} series={cci.series} decimals={1}
          legend={<><Lg c={C.ind} t={T("현재 CCI", "Current CCI")} b /><Lg c={C.emer} t={T("향후 12개월", "Next 12 mo.")} /></>}
      ...
      {bci.series.length > 0 && (
        <ChartCard seg="B2B" title={T("기업심리 BCI", "Business BCI")} unit={T("지수", "Index")} labels={bci.labels} series={bci.series} decimals={1}
          legend={<><Lg c={C.ind} t={T("현재 BCI", "Current BCI")} b /><Lg c={C.blue} t={T("향후 12개월", "Next 12 mo.")} /></>}
        ...
        { key: "bes", label: T("기업경기(BES) 전체분석", "Business Cycle (BES)"), node: <>
      {besOverall.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("BES 기업경기 종합지수", "BES Overall Index")} unit={T("확산지수 · 분기(2001~)", "Diffusion · Qtr (2001–)")} labels={besOverall.labels} series={besOverall.series} decimals={1} seriesUnit=""
          legend={<><Lg c={C.ind} t={T("당분기", "Current Q")} b /><Lg c={C.emer} t={T("차분기 전망", "Next-Q outlook")} /></>}
      ...
      {besSector.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("BES 업종별 신뢰", "BES by Sector")} unit={T("확산지수 · 분기", "Diffusion index · Quarterly")} labels={besSector.labels} series={besSector.series} decimals={1} seriesUnit=""
          legend={<><Lg c={C.rose} t={T("제조업", "Mfg")} b /><Lg c={C.ind} t={T("도소매", "Trade")} /><Lg c={C.emer} t={T("서비스", "Services")} /></>}
      ...
      {besJob.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("BES 고용전망·설비확장", "BES Jobs & Expansion")} unit={T("지수·% · 분기", "Index·% · Quarterly")} labels={besJob.labels} series={besJob.series} decimals={1} seriesUnit=""
          legend={<><Lg c={C.ind} t={T("고용전망(차분기)", "Emp. outlook (next Q)")} b /><Lg c={C.emer} t={T("설비확장 계획%", "Expansion plans %")} /></>}
      ...
      {besOps.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("BES 신용접근·재무여건", "BES Credit & Finances")} unit={T("확산지수 · 분기", "Diffusion index · Quarterly")} labels={besOps.labels} series={besOps.series} decimals={1} seriesUnit=""
          legend={<><Lg c={C.ind} t={T("신용접근", "Credit access")} b /><Lg c={C.rose} t={T("재무여건", "Fin. condition")} /></>}
      ...
      {besCap.series.length > 0 && (
        <ChartCard seg="B2B" title={T("BES 평균 가동률", "BES Avg Utilization")} unit={T("% · 분기", "% · Quarterly")} labels={besCap.labels} series={besCap.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.amber} t={T("평균 가동률(산업·건설)", "Avg utilization (ind·const)")} b />}
      ...
      {besCon.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("사업 제약요인 추이", "Constraints Trend")} unit={T("%응답 · 분기(복수)", "% resp · Qtr (multi)")} labels={besCon.labels} series={besCon.series} decimals={0} seriesUnit="%"
          legend={<><Lg c={C.rose} t={T("경쟁 심화", "Competition")} b /><Lg c={C.ind} t={T("수요 부족", "Weak demand")} /><Lg c={C.amber} t={T("고금리", "High rates")} /><Lg c={C.emer} t={T("재무 문제", "Financial problems")} /></>}
      ...
      {cesDur.series.length > 0 && (
        <ChartCard seg="CE" title={T("소비자 내구재 구매 (CES)", "Durables Bought (CES)")} unit={T("% 가구 · 분기", "% of households · Quarterly")} labels={cesDur.labels} series={cesDur.series} decimals={1} seriesUnit="%"
          legend={<Lg c={C.emer} t={T("내구재 구매 가구비중", "Durables-buying HH %")} b />}
...
export function PricesView() {
  ...
  const energy = build(d, n, [{ key: "INF_electricity", name: T("전기", "Elec."), color: C.ind, w: 2 }, { key: "INF_lpg", name: "LPG", color: C.rose }, { key: "INF_transport", name: T("운송", "Transport"), color: C.blue }])
  const home = build(d, n, [{ key: "INF_housing_utilities", name: T("주거·공공요금", "Housing & util."), color: C.ind, w: 2 }, { key: "INF_household_appliances", name: T("가전", "Appliances"), color: C.emer }, { key: "INF_aircon", name: T("에어컨", "Aircon"), color: C.rose }])
  const dine = build(d, n, [{ key: "INF_restaurants", name: T("외식·숙박", "Dining/lodging"), color: C.ind, w: 2 }, { key: "INF_all_items", name: T("전체 물가", "Headline"), color: C.brown }])
  const elec = build(d, n, [{ key: "meralco_residential_rate", name: T("가정용 전기료", "Res. elec."), color: C.ind, w: 2 }])
  ...
  return (
    <Shell title={T("물가", "Prices")} sub={T("생활물가·에너지·주거/내구재 CPI 상승률 — 실질 구매력·원가", "Cost-of-living, energy & housing/durables CPI growth — real purchasing power & cost")} win={win} setWin={setWin} loaded={loaded} empty={empty} d={d} accent="indigo"
      ...
      kpiDefs={[
        { key: "INF_all_items", label: T("전체 물가", "Headline"), fmt: (v) => v + "%", tone: "rose" },
        { key: "INF_food", label: T("식품", "Food"), fmt: (v) => v + "%", tone: "rose" },
        { key: "INF_rice", label: T("쌀", "Rice"), fmt: (v) => v + "%", tone: "rose" },
        { key: "INF_electricity", label: T("전기", "Elec."), fmt: (v) => v + "%", tone: "rose" },
      ...
      sections={[
        { key: "cpi", label: T("소비자물가", "Consumer CPI"), node: <>
      ...
      {food.series.length > 0 && (
        <ChartCard seg="CE" title={T("식품 물가", "Food CPI")} unit={T("전년비 %", "YoY %")} labels={food.labels} series={food.series} decimals={1} seriesUnit="%"
      ...
      {rice.series.length > 0 && (
        <ChartCard seg="CE" title={T("쌀 물가", "Rice CPI")} unit={T("전년비 %", "YoY %")} labels={rice.labels} series={rice.series} decimals={1} seriesUnit="%"
      ...
      {dine.series.length > 0 && (
        <ChartCard seg="CE" title={T("외식·숙박 vs 전체 물가", "Dining vs Headline")} unit={T("전년비 %", "YoY %")} labels={dine.labels} series={dine.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("외식·숙박", "Dining/lodging")} b /><Lg c={C.brown} t={T("전체 물가", "Headline")} /></>}
        ...
        { key: "energy", label: T("에너지·주거", "Energy·Home"), node: <>
      {energy.series.length > 0 && (
        <ChartCard seg="CE" title={T("에너지·이동 물가 (전기·LPG·운송)", "Energy Prices (Elec·LPG·Transp)")} unit={T("전년비 %", "YoY %")} labels={energy.labels} series={energy.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("전기", "Elec.")} b /><Lg c={C.rose} t="LPG" /><Lg c={C.blue} t={T("운송", "Transport")} /></>}
      ...
      {home.series.length > 0 && (
        <ChartCard seg="CE" title={T("주거·내구재 물가 (주거·가전·에어컨)", "Durables Prices (Home·Appl·AC)")} unit={T("전년비 %", "YoY %")} labels={home.labels} series={home.series} decimals={1} seriesUnit="%"
          legend={<><Lg c={C.ind} t={T("주거·공공요금", "Housing & util.")} b /><Lg c={C.emer} t={T("가전", "Appliances")} /><Lg c={C.rose} t={T("에어컨", "Aircon")} /></>}
      ...
      {elec.series.length > 0 && (
        ...
        { key: "oil", label: T("유가", "Fuel"), node: <>
      {oil.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("유가 (경유·휘발유·등유)", "Fuel Prices by Type")} unit={T("₱/L · 월별", "₱/L · Monthly")} labels={oil.labels} series={oil.series} decimals={1} seriesUnit="₱"
      ...
      {gas.series.length > 0 && (
        <ChartCard seg="CE·B2B" title={T("휘발유 등급별 (RON95·RON91)", "Gasoline by Grade (RON95/91)")} unit={T("₱/L · 월별", "₱/L · Monthly")} labels={gas.labels} series={gas.series} decimals={1} seriesUnit="₱"
