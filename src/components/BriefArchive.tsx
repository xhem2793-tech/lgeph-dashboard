  ...
  return (
    <section className="h-full rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 lg:col-span-2">
      <header className="mb-2 flex items-baseline justify-between">
        <h3 className="text-[15px] font-bold tracking-tight text-gray-900 dark:text-gray-50">지난 7일 브리핑</h3>
      ...
      {err ? (
        ...
        <div className="flex flex-col">
          {(rows ?? Array.from({ length: 3 })).map((r, i) =>
            !r ? (
              <div key={i} className="h-[30px] border-b border-gray-50 dark:border-gray-800" />
            ) : (
              <div
                key={r.asOf}
              ...
              >
                <span className="w-[52px] shrink-0 text-[12px] tabular-nums text-gray-400 dark:text-gray-500">
                  {fmt(r.asOf)}
                </span>
                <span
                  className={
                    "shrink-0 rounded px-1.5 py-px text-[11px] font-bold " +
                    (r.status === "approved"
                ...
                <p className="min-w-0 flex-1 truncate text-[13px] text-gray-700 dark:text-gray-200" title={r.head}>
                  {r.head || "—"}
                </p>
                <span className="shrink-0 text-[11px] text-gray-300 dark:text-gray-600">{r.nLines}줄</span>
              </div>
            ),
          )}
        </div>
      ...
      <p className="mt-2 border-t border-gray-100 dark:border-gray-800 pt-2 text-[11px] leading-snug text-gray-400 dark:text-gray-500">
