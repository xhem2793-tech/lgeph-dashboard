...
function DeltaCell({ d, dir, unit, muted }: { d: number | null; dir: string | null; unit: string | null; muted?: boolean }) {
  if (d == null)
    return (
      <span className="num w-[60px] shrink-0 text-right text-[10px] text-gray-300 dark:text-gray-600">
        —
      </span>
    )
  const up = d > 0
  ...
  return (
    <span
      className={
        "num inline-flex font-semibold w-[60px] shrink-0 items-center justify-end gap-0.5 rounded px-1 py-0.5 text-[10px] leading-4 " +
  ...
  return (
    <section className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm transition-shadow duration-300 hover:shadow-md">
      <header className="flex items-baseline justify-between border-b border-gray-100 dark:border-gray-800 px-3.5 py-2.5">
        ...
        <span
          key={mode}
          className="rounded-full bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 text-[11px] font-medium text-indigo-600 dark:text-indigo-400"
      ...
      {err ? (
        <p className="px-3 py-6 text-center text-[13px] text-gray-400 dark:text-gray-500">{t("rail_fail")}</p>
      ) : (
        GROUPS.map((g) => {
          ...
          return (
            <div key={g}>
              <p className="border-b border-gray-100 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-900/70 px-2.5 py-0.5 text-[11px] font-normal text-gray-400 dark:text-gray-500">{g === "월별" ? t("rail_monthly") : t("rail_quarterly")}</p>
                ...
                !c ? (
                  ...
                  <div key={c.key} className="mx-2.5 border-b border-dashed border-gray-300 dark:border-gray-700 last:border-b-0">
                    ...
                    >
                      <p
                        className={
                          "w-[104px] shrink-0 text-[14px] font-medium leading-snug text-gray-800 dark:text-gray-100 transition-colors duration-300 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 " +
                          (lang === "en" ? "truncate text-[13px]" : "truncate text-[14px]")
                        }
                      >
                      ...
                      <p className="num min-w-0 flex-1 whitespace-nowrap pr-2 text-right text-[13px] font-semibold text-gray-900 dark:text-gray-50 transition-colors duration-300 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">
    ...
    <div className="pb-2.5 pt-2.5" style={{ animation: "fadeUp .35s cubic-bezier(.22,1,.36,1) both" }}>
      <div className="rounded-xl bg-[#f9fafb] p-3">
        <div className="flex items-center justify-between gap-2">
          <span className={(lang === "en" ? "text-[13px]" : "text-[14px]") + " font-medium text-gray-800 dark:text-gray-100"}>{pick(c.label, c.labelEn)}</span>
        ...
        <p className="mt-0.5 flex items-baseline gap-1.5">
          <span className="num text-[21px] font-semibold text-gray-900 dark:text-gray-50">
            <CountUp value={Number(c.value)} prefix={c.prefix} suffix={c.suffix} decimals={dec} />
          </span>
          <span className="text-[11px] text-gray-400/90 dark:text-gray-500/90">{c.asOf?.slice(0, 7).replace("-", ".")} 기준</span>
        </p>

        {/* 변동률 라벨은 상세에서 본다 — 목록 배지에는 숫자만 */}
        <div className="mb-1 flex flex-wrap items-center gap-1.5 text-[11px] tabular-nums">
          {c.deltaMom != null ? (
        ...
        <div className="mt-2 border-t border-gray-200 dark:border-gray-800 pt-2">
          <span className="text-[11px] text-gray-400 dark:text-gray-500">출처 {c.freq === "분기" ? "BSP·PSA 분기" : "PSA·BSP 월별"} · {c.deltaLabel}</span>
        </div>
      </div>
    </div>
  )
