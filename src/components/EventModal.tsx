    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" style={{ animation: closing ? "evBackOut .22s ease both" : "evBackIn .22s ease both" }} onClick={close}>
      ...
      <div className="relative flex max-h-[88vh] w-full max-w-[600px] flex-col overflow-hidden rounded-2xl bg-white dark:bg-gray-900 shadow-2xl" style={{ animation: closing ? "evModalOut .22s cubic-bezier(.4,0,1,1) both" : "evModalIn .34s cubic-bezier(.22,1,.36,1) both" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto px-7 pb-7 pt-5">
          ...
          <h3 className="mt-2 text-[20px] font-semibold leading-snug tracking-tight text-gray-900 dark:text-gray-50">{m.event}</h3>
          ...
          {m.implication && (
            <div className="mt-4 border-l-2 border-indigo-300 dark:border-indigo-500/40 pl-3">
              ...
              <p className="mt-1 text-[14px] leading-[1.7] text-gray-800 dark:text-gray-100">{m.implication}</p>
            </div>
          )}
          ...
          {m.summary && (
            <div className="mt-4 space-y-2">
              ...
              {para(m.summary).map((p, k) => <p key={k} className="text-[13px] leading-[1.7] text-gray-600 dark:text-gray-300">{p}</p>)}
            </div>
          )}
          ...
          {m.actions && (
            <div className="mt-4">
              ...
              <p className="mt-1 whitespace-pre-line text-[13px] leading-[1.7] text-gray-600 dark:text-gray-300">{m.actions}</p>
            </div>
          )}

          {m.url && (
            <a href={m.url} target="_blank" rel="noreferrer" className="mt-5 flex items-center justify-center gap-1 rounded-lg bg-gray-900 py-2.5 text-[13px] font-semibold text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-gray-800 active:scale-[.99]">원문 보기 ↗</a>
          )}
        </div>
      </div>
    </div>,
