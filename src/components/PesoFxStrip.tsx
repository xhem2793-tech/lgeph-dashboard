...
export default function PesoFxStrip() {
  ...
  return (
    <div style={{ animation: "fadeUp .8s ease both" }}>
      <p className="mb-1.5 text-[13px] text-gray-500 dark:text-gray-400">
      ...
      <div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-200 dark:bg-gray-700 sm:grid-cols-3 lg:grid-cols-[repeat(4,1fr)_1.5fr]">
        {ORDER.map((k) => {
          ...
          return (
            <div key={k} className={cell}>
              <div className="text-[12px] text-gray-500 dark:text-gray-400">{p.label}</div>
              <div className="my-0.5 text-[18px] font-semibold tabular-nums text-gray-900 dark:text-gray-50">{fmt(k, p.rate)}</div>
              <div className={"text-[12px] font-medium tabular-nums " + col}>
        ...
        <div className={cell}>
          <div className="mb-1 text-[12px] text-gray-500 dark:text-gray-400">
          ...
          {peers.map((p, i) => {
            ...
            return (
              <div key={i} className="my-[3px] flex items-center gap-1.5">
                <span className={"w-9 shrink-0 text-[11px] " + (hl ? "font-medium text-rose-600 dark:text-rose-400" : "text-gray-500 dark:text-gray-400")}>{p.label}</span>
                ...
                <span className={"w-8 shrink-0 text-right text-[11px] tabular-nums " + (hl ? "text-rose-600 dark:text-rose-400" : "text-gray-400 dark:text-gray-500")}>
