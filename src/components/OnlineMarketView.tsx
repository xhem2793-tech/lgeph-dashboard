...
export default function OnlineMarketView() {
  ...
  const pay = build(d, [{ key: "credit_card_ownership", name: T("신용카드 보유", "Credit-card ownership"), color: C.rose }, { key: "debit_card_ownership", name: T("직불카드 보유", "Debit-card ownership"), color: C.blue }, { key: "credit_card_used", name: T("신용카드 사용", "Credit-card usage"), color: C.amber }])
  const secure = build(d, [{ key: "secure_internet_servers", name: T("보안 인터넷서버", "Secure servers"), color: C.violet }])
  const dpay = build(d, [{ key: "digital_payment_share", name: T("디지털 결제 비중", "Digital payment share"), color: C.emer }])
      ...
      <div className="grid items-start gap-4">
        <section className="min-w-0 rounded-xl p-4" style={{ animation: "fadeUp .34s cubic-bezier(.22,1,.36,1) both" }}>
          ...
          <div key={"ph" + win} className="grid items-stretch gap-4 sm:grid-cols-2" style={{ animation: "fadeUp .35s cubic-bezier(.22,1,.36,1) both" }}>
            ...
            {cPay.has && (
              <ChartCard seg="CE" title={T("온라인 결제수단 보급", "Online Payment Methods")} unit={T("% 성인 · 연간", "% of adults · annual")} labels={cPay.labels} series={cPay.series} decimals={1} seriesUnit="%"
                legend={<><Lg c={C.rose} t={T("신용카드 보유", "Credit-card ownership")} /><Lg c={C.blue} t={T("직불카드 보유", "Debit-card ownership")} /><Lg c={C.amber} t={T("신용카드 사용", "Credit-card usage")} /></>}
            ...
            {cInfra.has && (
              <ChartCard seg="CE" title={T("통신 인프라 침투", "Telecom Infrastructure Penetration")} unit={T("100명당 · 연간", "per 100 people · annual")} labels={cInfra.labels} series={cInfra.series} decimals={0} seriesUnit=""
          ...
          <div key={"sea" + win} className="grid items-stretch gap-4 sm:grid-cols-2" style={{ animation: "fadeUp .35s cubic-bezier(.22,1,.36,1) both" }}>
            ...
            <ChartCard seg="CE" title={T("금융계정 보유율 — 6개국", "Account Ownership — 6 countries")} unit={T("% 성인 · 연간", "% of adults · annual")} labels={cSa.labels} series={cSa.series} decimals={0} seriesUnit="%"
              legend={seaLegend}
            ...
            <ChartCard seg="CE" title={T("신용카드 보유율 — 6개국", "Credit-card Ownership — 6 countries")} unit={T("% 성인 · 연간", "% of adults · annual")} labels={cSc.labels} series={cSc.series} decimals={0} seriesUnit="%"
              legend={seaLegend}
            ...
            <ChartCard seg="CE" title={T("고정 브로드밴드 — 6개국", "Fixed Broadband — 6 countries")} unit={T("100명당 · 연간", "per 100 people · annual")} labels={cSb.labels} series={cSb.series} decimals={0} seriesUnit=""
              legend={seaLegend}
            ...
            <ChartCard seg="CE" title={T("모바일 가입 — 6개국", "Mobile Subscriptions — 6 countries")} unit={T("100명당 · 연간", "per 100 people · annual")} labels={cSm.labels} series={cSm.series} decimals={0} seriesUnit=""
              legend={seaLegend}
