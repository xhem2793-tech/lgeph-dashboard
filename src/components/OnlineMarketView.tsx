    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="overflow-hidden rounded-xl border border-violet-100 dark:border-violet-500/25 bg-gradient-to-r from-violet-50 dark:from-violet-500/10 via-violet-50/40 dark:via-transparent to-white dark:to-gray-900 shadow-sm" style={{ animation: "fadeOnly .5s ease both" }}>
        ...
        <div style={{ display: "grid", gridTemplateRows: open ? "1fr" : "0fr", transition: "grid-template-rows .36s cubic-bezier(.16,1,.3,1)" }}>
          <div className="overflow-hidden"><div className="border-t border-violet-100/70 dark:border-violet-500/25 px-4 pb-3.5 pt-3 text-[14.5px] leading-relaxed text-gray-700 dark:text-gray-200">
            ...
            <p className="mt-2 flex items-start gap-1.5 text-[14px] text-violet-700 dark:text-violet-300"><span className="mt-0.5 shrink-0 rounded bg-violet-600 px-1.5 py-0.5 text-[11.5px] font-bold text-white">LG 관점</span><span>온라인 전용 모델·D2C·마켓플레이스 플래그십 + 무이자할부/BNPL 제휴로 전환율 제고. 모바일 포화·브로드밴드 확대는 스마트가전 연결성 마케팅 기반.</span></p>
          </div></div>
        </div>
      </div>
      ...
      <div className="grid items-start gap-4 lg:grid-cols-[minmax(0,1fr)_286px]">
        <section className="min-w-0 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-sm" style={{ animation: "fadeUp .34s cubic-bezier(.16,1,.3,1) both" }}>
          <header className="mb-3 flex flex-wrap items-center gap-2.5 border-b border-gray-100 dark:border-gray-800 pb-2.5">
            ...
            <h2 className="text-[19px] font-bold tracking-tight text-gray-900 dark:text-gray-50">온라인 시장</h2>
            <span className="text-[13px] font-semibold text-gray-400 dark:text-gray-500">{tab === "sea" ? "동남아 6개국 비교 · ●필리핀 강조" : "이커머스·디지털·결제 저변"}</span>
          ...
          <div key={"ph" + win} className="grid items-stretch gap-4 sm:grid-cols-2" style={{ animation: "fadeUp .35s cubic-bezier(.16,1,.3,1) both" }}>
            {cDigital.has && (
              <ChartCard seg="CE" title="디지털 이용 확산" unit="% · 연간" labels={cDigital.labels} series={cDigital.series} decimals={0} seriesUnit="%"
                ...
                meaning={<>인터넷·금융계정 확대 — <b className="text-gray-700 dark:text-gray-200">온라인 구매·전자결제 저변</b></>}
            ...
            {cPay.has && (
              <ChartCard seg="CE" title="온라인 결제수단 보급" unit="% 성인 · 연간" labels={cPay.labels} series={cPay.series} decimals={1} seriesUnit="%"
                ...
                meaning={<>카드 보급(Findex) — <b className="text-gray-700 dark:text-gray-200">이커머스 결제·할부 구조</b></>}
            ...
            {cInfra.has && (
              <ChartCard seg="CE" title="통신 인프라 침투" unit="100명당 · 연간" labels={cInfra.labels} series={cInfra.series} decimals={0} seriesUnit=""
                ...
                meaning={<>모바일·브로드밴드 보급 — <b className="text-gray-700 dark:text-gray-200">모바일커머스·스마트가전 연결</b></>}
            ...
            {cDpay.has && (
              <ChartCard seg="CE" title="디지털 결제 비중" unit="% 소매결제 · 연간" labels={cDpay.labels} series={cDpay.series} decimals={0} seriesUnit="%"
                ...
                meaning={<>디지털 결제(e-wallet) 비중 — <b className="text-gray-700 dark:text-gray-200">GCash·Maya 확산 직접 지표</b></>}
            ...
            {cSecure.has && (
              <ChartCard seg="B2B" title="이커머스 인프라 (보안서버)" unit="백만명당 · 연간" labels={cSecure.labels} series={cSecure.series} decimals={0} seriesUnit=""
                ...
                meaning={<>보안서버 밀도 — <b className="text-gray-700 dark:text-gray-200">전자상거래 신뢰 인프라 성숙도</b></>}
      ...
      <p className="text-[13px] leading-relaxed text-gray-400 dark:text-gray-500">출처 디지털/결제/인프라·6개국 비교=World Bank(WDI·Global Findex)·ITU 연간 실측 · 신용카드·계정 보유율은 격년 서베이(2011~2024) · 인터넷은 WB·ITU 기준(교차국 비교 일치)</p>
    </div>
  )
}
