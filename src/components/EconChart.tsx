...
export function BarChart({ data, labels, color = IND, decimals = 1, unit = "" }: { data: number[]; labels: string[]; color?: string; decimals?: number; unit?: string }) {
  ...
  React.useEffect(() => {
    ...
    const head = document.createElement("div"); head.className = "mb-0.5 text-[13px] font-medium text-gray-400 dark:text-gray-500"; tip.appendChild(head)
    const valRow = document.createElement("div"); valRow.className = "text-[15px] font-bold tabular-nums"; tip.appendChild(valRow)
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5">
        ...
        {seg && <span className={"shrink-0 rounded px-1.5 py-0.5 text-[11px] font-bold " + (seg === "B2B" ? "bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300" : seg === "CE" ? "bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300" : "bg-violet-50 dark:bg-violet-500/10 text-violet-700")}>{seg}</span>}
        {unit && <span className="ml-auto shrink-0 text-[12.5px] font-medium text-gray-400 dark:text-gray-500">{unit}</span>}
