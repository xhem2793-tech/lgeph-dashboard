...
export function LineChart({ series, labels, decimals = 1, unit = "" }: { series: SLine[]; labels: string[]; decimals?: number; unit?: string }) {
  ...
  React.useEffect(() => {
    ...
    const L = 36, R = 292, T = 6, B = 72 // 세로길이 확대(plot 하단 58→72). 끝점 라벨은 차트 밖으로 넘김(overflow visible)
    ...
    const loop = () => {
      ...
      const sx = rectW / 300, sy = rectH / 88
  ...
  return (
    <div className="relative mt-1 mx-auto w-full max-w-[900px]" style={{ touchAction: "none" }}>
      <svg ref={svgRef} viewBox="0 0 300 88" width="100%" style={{ height: "auto", display: "block", cursor: "crosshair", overflow: "visible" }} />
...
export function BarChart({ data, labels, color = IND, decimals = 1, unit = "" }: { data: number[]; labels: string[]; color?: string; decimals?: number; unit?: string }) {
  ...
  React.useEffect(() => {
    ...
    const n = data.length
    if (!n) return
    const L = 36, R = 292, T = 6, B = 72 // 세로길이 확대(58→72)
    const el = (t: string, a: Record<string, string | number>) => { const e = document.createElementNS(NS, t); for (const k in a) e.setAttribute(k, String(a[k])); return e }
  ...
  return (
    <div className="relative mt-1 mx-auto w-full max-w-[900px]" style={{ touchAction: "none" }}>
      <svg ref={svgRef} viewBox="0 0 300 88" width="100%" style={{ height: "auto", display: "block", cursor: "crosshair" }} />
