...
// "25.1" → "Jan 25"(축), "Jan 2025"(툴팁) — 가독성
const fxLbl = (lb: string) => { const [y, m] = String(lb).split("."); return m ? (MON[Number(m) - 1] || "?") + " " + y : lb }
const fxLblFull = (lb: string) => { const [y, m] = String(lb).split("."); return m ? (MON[Number(m) - 1] || "?") + " 20" + y : lb }
...
function FxChart({ series, labels, decimals = 1, unit = "" }: { series: SLine[]; labels: string[]; decimals?: number; unit?: string }) {
  ...
  React.useEffect(() => {
    ...
    labels.forEach((lb, i) => { if (!showSet.has(i)) return; const an = i <= 1 ? "start" : i >= n - 2 ? "end" : "middle"; const tx = el("text", { x: X(i), y: B + 13, "text-anchor": an, "font-size": 9.5, fill: "#9ca3af" }); tx.textContent = fxLbl(lb); svg.appendChild(tx) })
    ...
    const place = (i: number) => {
      active = i; head.textContent = fxLblFull(labels[i])
      const tops: number[] = []
      series.forEach((s, si) => {
