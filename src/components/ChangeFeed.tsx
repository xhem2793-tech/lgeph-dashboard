    ...
    <section className="h-full rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4">
      <header className="mb-2 flex items-baseline justify-between">
        <div className="flex items-baseline gap-2">
          ...
          <span className="text-[12px] text-gray-400 dark:text-gray-500">최근 7일 · {fmtDate(asOf)} 기준</span>
        </div>
        <a
          href="/competitors"
          className="text-[12px] text-gray-400 dark:text-gray-500 transition-colors duration-200 hover:text-indigo-600 dark:hover:text-indigo-400"
        >
          전체 &gt;
        </a>
      </header>
          ...
          {(rows ?? Array.from({ length: 5 })).map((r, i) =>
            !r ? (
              ...
              >
                ...
                <p className="min-w-0 flex-1 text-[13px] leading-snug text-gray-700 dark:text-gray-200">
                ...
                <span className="shrink-0 text-[11px] text-gray-300 dark:text-gray-600">{KIND[r.kind]}</span>
              </div>
            ),
          )}
        </div>
