...
function BrandLogo({ brand }: { brand: string }) {
  ...
  return (
    <span className="inline-flex h-7 w-[54px] items-center justify-center align-middle transition-all duration-300 ease-out hover:-translate-y-0.5">
      ...
      <span
        className="rounded px-1 py-0.5 text-[12px] font-bold text-white"
...
export default function CompetitorMovers() {
  ...
  const th = "px-1 py-0.5 text-center text-[12px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500"
  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3 shadow-sm transition-shadow duration-300 hover:shadow-md" style={{ animation: "fadeUp .5s cubic-bezier(.22,1,.36,1) both", animationDelay: "0.6s" }}>
      ...
      <div className="mb-1 flex flex-wrap items-center justify-between gap-x-3 gap-y-1 px-0.5">
        <div className="flex items-center gap-2">
          <a href="/competitors" className="group flex items-baseline gap-1">
            <h2 className="text-[19px] font-bold tracking-tight text-gray-900 dark:text-gray-50 transition-colors duration-300 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">{t("price_title")}</h2>
          ...
          <span className="inline-flex cursor-default items-center gap-1 rounded-full bg-emerald-50 dark:bg-emerald-500/10 px-2 py-0.5 text-[12px] font-semibold text-emerald-600 dark:text-emerald-400 transition-all duration-300 ease-out hover:text-emerald-700 dark:hover:text-emerald-300">
            <span className="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" />
            {t("price_daily")}
          </span>
        </div>
        <span className={HOVM + " flex items-center gap-1.5 text-[12px] text-gray-400 dark:text-gray-500"}>
      ...
      <div className="flex h-full flex-col rounded-xl bg-[#f9fafb] p-2.5 transition-all duration-300 ease-out hover:-translate-y-0.5 hover:bg-white dark:hover:bg-gray-900 hover:shadow-md">
          ...
          <div className="flex flex-wrap gap-1">
            {cats.map((c) => (
              <button
                ...
                type="button"
                onClick={() => pick(c)}
                className={"shrink-0 whitespace-nowrap rounded px-1.5 py-0.5 text-[12px] font-medium transition-all duration-200 active:scale-95 " + (cat === c ? "bg-indigo-600 text-white shadow-sm" : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:-translate-y-0.5 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-indigo-600 dark:hover:text-indigo-400")}
              >
                {CAT_LABEL[c] ?? c}
              </button>
            ))}
        ...
        <div className="mt-1 rounded-lg border border-indigo-100/70 dark:border-indigo-500/25 bg-white/70 dark:bg-gray-900/70 px-2 py-1.5 transition-all duration-300 ease-out hover:-translate-y-0.5 hover:border-indigo-300 dark:hover:border-indigo-500/40 hover:bg-white dark:hover:bg-gray-900">
          <div>
            <div
            ...
            >
              <div className="min-h-[167px] w-full max-w-full overflow-hidden">
                  ...
                  <tbody>
                    {cardRows.length === 0 ? (
                      <tr className="border-b border-gray-100 dark:border-gray-800">
                        <td className={td} colSpan={7}><div className={cell + " text-[13px] text-gray-400 dark:text-gray-500"}>{t("price_none")}</div></td>
                      </tr>
                    ) : null}
                    {cardRows.map((r, i) => (
                      <tr key={`${cat}-${sortDir}-${i}`} style={{ animation: "calIn .5s cubic-bezier(.16,1,.3,1) backwards", animationDelay: i * 0.1 + "s" }} className="border-b border-gray-100 dark:border-gray-800 transition-colors duration-200 hover:bg-indigo-50/40 dark:hover:bg-indigo-500/10">
                        <td className={td}><div className={cell}><BrandLogo brand={r.brand} /></div></td>
                        <td className={td}>
                          <div className={cell}>
                            <span className={HOVM + " whitespace-nowrap rounded bg-gray-100 dark:bg-gray-800 px-1 text-[12px] font-semibold leading-[16px] text-gray-500 dark:text-gray-400"}>{CAT_LABEL[r.category] ?? r.category}</span>
                          </div>
                        </td>
                        <td className={td}>
                          <div className={cell}>
                            <span className={HOV + " truncate font-medium leading-none text-gray-700 dark:text-gray-200"} title={r.model}>{modelCode(r.model, r.brand)}</span>
                          </div>
                        </td>
                        ...
                        <td className={td}><div className={cell}><span className={HOVM + " whitespace-nowrap text-[12px] leading-none text-gray-500 dark:text-gray-400"}>{shopName(r.retailer)}</span></div></td>
                      </tr>
                    ))}
                    {/* 행 수는 항상 5줄 — 표 높이가 날마다 출렁이면 눈이 위치를 다시 찾는다 */}
                    {Array.from({ length: Math.max(0, (cardRows.length === 0 ? 4 : 5) - cardRows.length) }).map((_, j) => (
                      <tr key={`pad-${cat}-${sortDir}-${j}`} aria-hidden className="border-b border-gray-100 dark:border-gray-800">
                ...
                <p className="mt-2 flex items-center gap-2 border-t border-gray-100 dark:border-gray-800 pt-1.5 text-[13px] text-gray-600 dark:text-gray-300">
