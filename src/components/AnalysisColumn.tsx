  ...
  return (
    <div className="space-y-3">
      {blocks.map((b, i) => {
        ...
        if (h) {
          return <h4 key={i} className="mt-4 text-[15px] font-semibold text-gray-900 dark:text-gray-50">{inline(h[2])}</h4>
        }
        return (
          <p key={i} className="whitespace-pre-wrap text-[14px] leading-relaxed text-gray-700 dark:text-gray-200">{inline(b)}</p>
        )
      })}
    </div>
  )
  ...
  return createPortal(
    <div className={SHELL} onClick={onClose}>
      ...
      <div className={CARD} style={ANIM} onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="text-[24px] font-bold leading-tight text-gray-900 dark:text-gray-50">{pick(r.title, r.titleEn)}</h2>
  ...
  return createPortal(
    <div className={SHELL} onClick={onClose}>
      ...
      <div className={CARD} style={ANIM} onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="text-[24px] font-bold leading-tight text-gray-900 dark:text-gray-50">{pick(p.title, p.titleEn)}</h2>
        {p.dek ? <p className="mt-1.5 text-[14px] leading-relaxed text-gray-500 dark:text-gray-400">{pick(p.dek, p.dekEn)}</p> : null}
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-[1fr_2fr]">
          ...
          <div>
            {p.summary ? (
              <p className="text-[14px] leading-relaxed text-gray-700 dark:text-gray-200">{pick(p.summary, p.summaryEn)}</p>
            ) : null}
    ...
    <div className="flex flex-col gap-4 lg:px-3">
      <section>
        <a href="/news?cat=분석" className="group mb-2 flex items-baseline gap-1">
          <span className="text-[16px] font-bold tracking-tight text-gray-900 dark:text-gray-50 transition-colors duration-300 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">
            {t("analysis_title")}
          </span>
        ...
        {err ? (
          ...
          <button type="button" onClick={() => setOpen(post)} className={CARD_BTN}>
            ...
            <p className="line-clamp-2 text-[14px] font-semibold leading-snug text-gray-800 dark:text-gray-100 transition-colors duration-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">
