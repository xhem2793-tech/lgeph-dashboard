  ...
  return (
    <div ref={cardRef} className="group/card relative z-0 flex h-full flex-col rounded-xl bg-white dark:bg-gray-900 p-3.5 transition-all duration-300 ease-out hover:z-30 hover:-translate-y-0.5 hover:shadow-md" style={{ animation: "fadeUp .5s cubic-bezier(.16,1,.3,1) both" }}>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="shrink-0 rounded bg-indigo-50 dark:bg-indigo-500/10 px-1.5 py-0.5 text-[10px] font-bold text-indigo-700 dark:text-indigo-300">CE·B2B</span>
        <span className="ml-auto shrink-0 text-[11.5px] font-medium text-gray-400 dark:text-gray-500">% · 합계 100%</span>
      ...
      <div className="mt-1.5 flex min-h-[30px] flex-wrap items-center gap-x-3 gap-y-1 text-[11.5px]">
        ...
        <div className="relative mt-1">
          ...
          {hi != null && byYear[hi] && (
            <div className="pointer-events-none absolute -top-1 right-0 z-10 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-2 py-1 shadow-lg">
              <div className="mb-0.5 text-[11px] font-bold text-gray-900 dark:text-gray-50">{hi}년</div>
              {MAJ.map((m) => <div key={m.key} className="flex items-center justify-between gap-2.5 text-[11px]"><span className="flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-sm" style={{ background: m.color }} /><span className="text-gray-500 dark:text-gray-400">{m.key}</span></span><span className="font-semibold tabular-nums text-gray-800 dark:text-gray-100">{((byYear[hi][m.key] || 0) * 100).toFixed(1)}%</span></div>)}
            </div>
          )}
        </div>
      )}
      <p className="mt-2.5 line-clamp-2 min-h-[34px] text-[12px] leading-relaxed text-gray-500 dark:text-gray-400"><b className="font-semibold text-gray-700 dark:text-gray-200">의미</b> 서비스업 비중 확대 = 서비스 중심 경제 · 도시 중산층 소비 저변</p>
      <p className="mt-2 line-clamp-2 min-h-[34px] border-l-2 border-indigo-300 dark:border-indigo-500/40 pl-2.5 text-[12px] leading-relaxed text-gray-600 dark:text-gray-300"><b className="font-semibold text-indigo-600 dark:text-indigo-400">LG 인사이트</b> 서비스화·도시화 심화는 <b className="font-semibold text-emerald-600 dark:text-emerald-400">도시가구·사무공간 냉난방·소형가전 수요 저변</b> → 도시 프리미엄·B2B 채널 우선</p>
      <p className="mt-auto border-t border-gray-100 dark:border-gray-800 pt-2 text-[11px] leading-relaxed text-gray-400 dark:text-gray-500"><b className="font-semibold text-gray-500 dark:text-gray-400">자료</b> PSA 국민계정 산업별 GDP 비중 · 연간 · 3대 산업 합계 100%</p>
    </div>
  )
}
