...
export default function GdpComposition() {
  ...
  const W = 300, H = 100, L = 16, R = 4, TP = 4, B = 12
  ...
  const Y = (frac: number) => TP + (H - TP - B) * (1 - frac)
  const gray = "#94a3b8"
  ...
  return (
    <div ref={cardRef} className="group/card relative z-0 flex h-full flex-col rounded-xl p-3.5 transition-all duration-300 ease-out hover:z-30 hover:-translate-y-0.5 hover:shadow-md" style={{ animation: "fadeUp .5s cubic-bezier(.22,1,.36,1) both" }}>
      ...
      <div className="flex items-center gap-1.5">
        <h3 className="text-[13.5px] font-bold tracking-tight text-gray-900 dark:text-gray-50">{T("GDP 산업구조", "GDP by Industry")}</h3>
        ...
        <span className="ml-auto shrink-0 text-[10.5px] font-medium text-gray-400 dark:text-gray-500">{T("% · 합계 100%", "% · total 100%")}</span>
        <span className="flex shrink-0 items-center gap-0.5 ml-1.5">
          <button type="button" onClick={dlImg} title={T("이미지(SVG) 다운로드", "Download image (SVG)")} className="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-indigo-600 dark:hover:bg-gray-800 dark:hover:text-indigo-400"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" /><circle cx="8.5" cy="8.5" r="1.5" /><path d="M21 15l-5-5L5 21" /></svg></button>
          <button type="button" onClick={dlCsv} title={T("데이터(CSV) 다운로드", "Download data (CSV)")} className="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-indigo-600 dark:hover:bg-gray-800 dark:hover:text-indigo-400"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3v12" /><path d="M8 11l4 4 4-4" /><path d="M4 21h16" /></svg></button>
        </span>
      </div>
        ...
        <div className="relative mt-1 mx-auto w-full max-w-[900px]">
            ...
            {years.map((yr, i) => {
              ...
              return (
                <g key={yr} onMouseEnter={() => setHi(yr)} opacity={hi == null || hi === yr ? 1 : 0.45} style={{ transition: "opacity .2s" }}>
                  <rect x={X(i) - bw / 2 - 1.5} y={TP} width={bw + 3} height={H - TP - B} fill="transparent" />
                  {MAJ.map((m, mi) => { const frac = (d[m.key] || 0) / total, h = (H - TP - B) * frac, y = Y(acc + frac); acc += frac
          ...
          {hi != null && byYear[hi] && (
            <div className="pointer-events-none absolute -top-1 right-0 z-10 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-2.5 py-1.5 shadow-lg">
              <div className="mb-0.5 text-[10px] font-bold text-gray-900 dark:text-gray-50">{hi}{T("년", "")}</div>
      ...
      <p className="mt-2.5 line-clamp-2 min-h-[34px] text-[11px] leading-relaxed text-gray-500 dark:text-gray-400"><b className="font-semibold text-gray-700 dark:text-gray-200">{T("의미", "Meaning")}</b>{T(" 서비스업 비중 확대 = 서비스 중심 경제 · 도시 중산층 소비 저변", " A rising services share = a services-led economy · a consumption base among the urban middle class")}</p>
      <p className="mt-2 line-clamp-2 min-h-[34px] border-l-2 border-indigo-300 dark:border-indigo-500/40 pl-2.5 text-[11px] leading-relaxed text-gray-600 dark:text-gray-300"><b className="font-semibold text-indigo-600 dark:text-indigo-400">{T("LG 인사이트", "LG Insight")}</b>{T(" 서비스화·도시화 심화는 ", " Deepening servicization and urbanization builds ")}<b className="font-semibold text-emerald-600 dark:text-emerald-400">{T("도시가구·사무공간 냉난방·소형가전 수요 저변", "a demand base for household and office HVAC and small appliances")}</b>{T(" → 도시 프리미엄·B2B 채널 우선", " → prioritize urban premium and B2B channels")}</p>
      <p className="mt-auto border-t border-gray-100 dark:border-gray-800 pt-2 text-[10px] leading-relaxed text-gray-400 dark:text-gray-500"><b className="font-semibold text-gray-500 dark:text-gray-400">{T("자료", "Source")}</b>{T(" PSA 국민계정 산업별 GDP 비중 · 연간 · 3대 산업 합계 100%", " PSA national accounts, GDP share by industry · annual · three major industries sum to 100%")}</p>
    </div>
  )
}
