...
const CAT: Record<string, { bg: string; fg: string; dot: string; band: string }> = {
  경제: { bg: "bg-emerald-50 dark:bg-emerald-500/10", fg: "text-emerald-800", dot: "bg-emerald-500", band: "bg-emerald-100 dark:bg-emerald-500/15 text-emerald-900" },
  금융: { bg: "bg-blue-50 dark:bg-blue-500/10", fg: "text-blue-800", dot: "bg-blue-500", band: "bg-blue-100 dark:bg-blue-500/15 text-blue-900" },
  정치: { bg: "bg-purple-50", fg: "text-purple-800", dot: "bg-purple-500", band: "bg-purple-100 text-purple-900" },
  규제: { bg: "bg-red-50 dark:bg-red-500/10", fg: "text-red-800", dot: "bg-red-500", band: "bg-red-100 dark:bg-red-500/15 text-red-900" },
  에너지: { bg: "bg-amber-50 dark:bg-amber-500/10", fg: "text-amber-800", dot: "bg-amber-500", band: "bg-amber-100 dark:bg-amber-500/15 text-amber-900" },
  유통: { bg: "bg-violet-50 dark:bg-violet-500/10", fg: "text-violet-800", dot: "bg-violet-500", band: "bg-violet-100 text-violet-900" },
  ...
  return (
    <div className="w-full px-6 pb-6 pt-4 sm:px-8 lg:px-10 sm:pb-8">
        ...
        >
          <header className="relative flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 dark:border-gray-800 pb-2.5">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-[20px] font-bold tracking-tight text-gray-900 dark:text-gray-50">{label}</h2>
              <span className="text-[14px] font-medium text-gray-500 dark:text-gray-400">{inMonth.length}건 · Critical {crit}건</span>
              {month !== 0 && (
                <button
                  type="button"
                  onClick={() => setMonth(0)}
                  className="rounded-full bg-indigo-50 dark:bg-indigo-500/10 px-2.5 py-0.5 text-[13px] font-semibold text-indigo-700 dark:text-indigo-300 transition-all duration-300 hover:-translate-y-px active:scale-95"
                >
                  {span === "2주" ? "이번 주" : "이번 달"}
                </button>
              )}
          ...
          <div className="mt-3 grid grid-cols-7 gap-1.5 text-[13.5px] font-semibold text-gray-400 dark:text-gray-500">
            {["일", "월", "화", "수", "목", "금", "토"].map((d) => (
              ...
              return (
                <div
                  ...
                  className={
                    ...
                    (isToday ? "border-indigo-400 dark:border-indigo-500/50 bg-indigo-50/40 dark:bg-indigo-500/10 " : holiday && on ? "border-teal-200 bg-teal-50/40 " : on ? "border-gray-200 dark:border-gray-800 " : "")
                  }
                >
                  {on && (
                    <>
                      ...
                      {evs.slice(0, span === "2주" ? 6 : 3).map((e) => {
                        ...
                        return (
                          <button
                            ...
                            className={
                              "mb-0.5 flex w-full items-center gap-1.5 rounded px-1.5 py-1 text-left text-[13.5px] font-medium leading-tight text-gray-700 dark:text-gray-200 transition-all duration-300 hover:bg-gray-100 dark:hover:bg-gray-800 active:scale-[.97] "
                            }
                            title={e.event}
                          >
                      ...
                      {evs.length > (span === "2주" ? 6 : 3) && (
                        <span className="block px-1 text-[12.5px] font-medium text-gray-400 dark:text-gray-500">+{evs.length - (span === "2주" ? 6 : 3)}건</span>
                    )}
                    </>
                  )}
                </div>
          ...
          <div className="mt-3 flex flex-wrap gap-3 text-[13.5px] font-medium text-gray-500 dark:text-gray-400">
            {LEGEND.map((c) => (
      ...
      >
        <header className="relative flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 dark:border-gray-800 pb-2.5">
          <div className="flex items-center gap-3">
            <h2 className="text-[20px] font-bold tracking-tight text-gray-900 dark:text-gray-50">이벤트 목록</h2>
            <Segmented size="sm"
              options={[
            ...
            />
              ...
              <div
                ...
                style={{ marginTop: "-5px" }}
              >
                <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 transition-colors duration-300 group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400">
                ...
                <input
                  ...
                  className="w-full rounded-full border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 py-1 pl-9 pr-9 text-[14px] outline-none transition-all duration-300 ease-out placeholder:text-gray-400 dark:placeholder:text-gray-500 hover:border-gray-300 dark:hover:border-gray-700 hover:bg-white dark:hover:bg-gray-900 focus:border-indigo-400 dark:focus:border-indigo-500/50 focus:bg-white dark:focus:bg-gray-900 focus:shadow-[0_0_0_3px_rgba(99,102,241,0.12)]"
                />
                {query && (
                  <button
                    type="button"
          ...
          <span className="flex items-center gap-1.5 text-[13px] text-gray-400 dark:text-gray-500">최근 갱신 {stamp ? fmtStamp(stamp) : "—"}<span className="rounded bg-emerald-50 dark:bg-emerald-500/10 px-1 py-px text-[11px] font-bold text-emerald-600 dark:text-emerald-400">C</span></span>
        </header>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          ...
          <span className="mx-1 h-5 w-px self-center bg-gray-200 dark:bg-gray-700" />
          {cats.map((c) => (
            <button
              key={c}
              type="button"
              onClick={() => setCat(c)}
              className={
                "rounded-md px-2.5 py-1 text-[14px] font-semibold transition-all duration-300 hover:-translate-y-px active:scale-[.98] " +
        ...
        {todayEvents.length > 0 && (
          <div className="mt-3 overflow-hidden rounded-xl border border-indigo-200 dark:border-indigo-500/40 bg-gradient-to-r from-indigo-50 dark:from-indigo-500/10 to-white dark:to-gray-900 shadow-sm" style={{ animation: "rowIn .5s cubic-bezier(.16,1,.3,1) both" }}>
            <div className="flex items-center gap-2 border-b border-indigo-100 dark:border-indigo-500/25 px-3.5 py-2">
              <span className="flex h-5 items-center rounded-full bg-indigo-600 px-2 text-[12.5px] font-bold text-white">오늘</span>
              <span className="text-[15px] font-bold text-gray-900 dark:text-gray-50">{todayIso.slice(5).replace("-", "/")} 발표·일정</span>
              <span className="text-[13px] font-medium text-indigo-600 dark:text-indigo-400">{todayEvents.length}건</span>
            </div>
              ...
              {todayEvents.map((e) => {
                ...
                return (
                  <button key={e.date + e.event} type="button" onClick={() => openEvent(e)} className="flex items-center gap-2.5 px-3.5 py-2 text-left transition-colors hover:bg-indigo-50/60 dark:hover:bg-indigo-500/10">
                    ...
                    <span className="shrink-0 text-[12.5px] font-semibold text-gray-500 dark:text-gray-400">{catLabel(e.category)}</span>
                    <span className="min-w-0 flex-1 truncate text-[15px] font-semibold text-gray-900 dark:text-gray-50">{e.importance >= 3 ? "★ " : ""}{e.event}</span>
                    {e.actual !== null ? (
                      <span className={"shrink-0 text-[14px] font-bold tabular-nums " + (up ? "text-rose-600 dark:text-rose-400" : down ? "text-emerald-600 dark:text-emerald-400" : "text-gray-900 dark:text-gray-50")}>{fmtVal(e.actual, e.unit)}</span>
                    ) : e.releaseTime ? <span className="shrink-0 text-[13px] text-gray-500 dark:text-gray-400">{e.releaseTime}</span> : <span className="shrink-0 rounded bg-indigo-100 dark:bg-indigo-500/20 px-1.5 py-px text-[11.5px] font-bold text-indigo-700 dark:text-indigo-300">예정</span>}
                  </button>
                )
              })}
            </div>
        ...
        {rows === null ? (
          <div className="flex min-h-[240px] items-center justify-center text-[15px] text-gray-400 dark:text-gray-500">불러오는 중</div>
        ) : list.length === 0 ? (
          <div className="flex min-h-[200px] items-center justify-center text-[15px] text-gray-400 dark:text-gray-500">해당 구간 이벤트 없음</div>
        ) : (
          <div key={bucket + cat + axis + query} className="mt-2 overflow-x-auto" style={{ animation: "viewIn .42s cubic-bezier(.16,1,.3,1) both" }}>
            <table className="w-full min-w-[760px] text-[15px]">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800 text-[13px] font-semibold text-gray-500 dark:text-gray-400">
                ...
                {list.map((e, i) => {
                  ...
                  return (
                    <React.Fragment key={e.date + e.event}>
                      {showHead && (
                        <tr>
                          <td colSpan={8} className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-900/60 px-2 pb-1.5 pt-2.5 text-[13px] font-bold text-gray-500 dark:text-gray-400 first:border-t-0">
                            {groupOf(e)}
                          </td>
                        </tr>
                      )}
                      ...
                      >
                        ...
                        <td className="h-[44px] px-2 align-middle">
                          <span className="flex items-center gap-1.5"><span className={"h-1.5 w-1.5 shrink-0 rounded-full " + t.dot} /><span className="text-[13.5px] text-gray-600 dark:text-gray-300">{catLabel(e.category)}</span></span>
                        </td>
                        <td className="h-[44px] px-2 align-middle text-[13.5px] text-gray-500 dark:text-gray-400">{KIND[e.kind] ?? "—"}</td>
                        <td className="max-w-0 truncate h-[44px] px-2 align-middle font-medium text-gray-900 dark:text-gray-50">{e.event}</td>
                        <td className="h-[44px] px-2 align-middle text-right text-[13px] text-amber-500 dark:text-amber-400">{"★".repeat(e.importance)}</td>
        ...
        <p className="mt-3 text-[13px] leading-relaxed text-gray-400 dark:text-gray-500">
        ...
        <div className="flex flex-col gap-4" style={{ animation: "fadeUp .5s ease both", animationDelay: "80ms" }}>
          <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-sm">
            <header className="flex items-baseline justify-between border-b border-gray-100 dark:border-gray-800 pb-2.5">
              <h2 className="text-[18px] font-bold tracking-tight text-gray-900 dark:text-gray-50">{label} 구성</h2>
              <span className="text-[13px] text-gray-400 dark:text-gray-500">최종 갱신 {stamp ? fmtStamp(stamp) : "—"}</span>
            </header>
            <div className="mt-3 grid grid-cols-3 gap-2 text-center">
              {[
                { k: "release", n: mix.release, c: "text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-500/10" },
                { k: "policy", n: mix.policy, c: "text-gray-700 dark:text-gray-200 bg-gray-50 dark:bg-gray-900" },
                { k: "holiday", n: mix.holiday, c: "text-teal-700 bg-teal-50" },
              ].map((x) => (
                <div key={x.k} className={"rounded-lg py-3 " + x.c}>
                  <p className="text-[23px] font-bold tabular-nums">{x.n}</p>
                  <p className="mt-0.5 text-[13px] font-medium">{KIND[x.k]}</p>
                </div>
              ))}
            </div>
            <p className="mt-3 text-[13px] leading-relaxed text-gray-400 dark:text-gray-500">
              ...
              <div className="mt-4 border-t border-gray-100 dark:border-gray-800 pt-3">
                <div className="mb-1 flex items-baseline justify-between">
                  <h3 className="text-[14px] font-bold text-gray-700 dark:text-gray-200">예정 일정</h3>
                  ...
                  {agenda.map((x, i) => {
                    ...
                    return (
                      <div key={x.label + x.date} onClick={() => x.ev && openEvent(x.ev)} style={{ animation: "rowIn .5s cubic-bezier(.16,1,.3,1) backwards", animationDelay: Math.min(i, 10) * 0.04 + "s" }} className={"flex items-start gap-2.5 rounded-lg px-1.5 py-2 transition-all duration-200 hover:bg-indigo-50/40 dark:hover:bg-indigo-500/10 " + (x.ev ? "cursor-pointer hover:-translate-y-px active:scale-[.99]" : "")}>
                        ...
                        <span className="shrink-0 tabular-nums text-[13px] font-semibold text-gray-500 dark:text-gray-400">{dd === 0 ? "오늘" : "D-" + dd}</span>
                      </div>
                    )
                  })}
                  {agenda.length === 0 && <p className="px-1.5 py-3 text-[13px] text-gray-400 dark:text-gray-500">2주간 예정된 일정 없음</p>}
                </div>
              </div>
          </div>
        ...
        >
          {modal ? (
            ...
            >
              ...
              <div className="flex w-full shrink-0 items-center gap-2 border-b border-gray-100 dark:border-gray-800 px-7 pb-3 pt-6 text-[14px] font-semibold">
                ...
                {modal.importance >= 2 && (
                  <span className="text-[14px] text-amber-500 dark:text-amber-400">{"★".repeat(modal.importance)}</span>
                )}
              </div>

              <div className="overflow-y-auto px-7 pb-7 pt-5">
                <div className="flex flex-wrap items-center gap-1.5 text-[13.5px] text-gray-500 dark:text-gray-400">
                ...
                <h3 className="mt-2 text-[23px] font-semibold leading-snug tracking-tight text-gray-900 dark:text-gray-50">{modal.event}</h3>

                {modal.indicatorKey && (
                  <div className="mt-4 inline-flex flex-wrap gap-4 rounded-lg bg-gray-50 dark:bg-gray-900 px-3.5 py-2 text-[14px] tabular-nums">
                ...
                {modal.implication && (
                  <div className="mt-4 border-l-2 border-indigo-300 dark:border-indigo-500/40 pl-3">
                    ...
                    <p className="mt-1 text-[16px] leading-[1.7] text-gray-800 dark:text-gray-100">{modal.implication}</p>
                  </div>
                )}
                ...
                {modal.summary && (
                  <div className="mt-4 space-y-2">
                    ...
                    {para(modal.summary).map((p, k) => (
                      <p key={k} className="text-[15px] leading-[1.7] text-gray-600 dark:text-gray-300">{p}</p>
                    ))}
                  </div>
                )}
                ...
                {modal.actions && (
                  <div className="mt-4">
                    <p className="text-[12.5px] font-semibold tracking-wide text-gray-400 dark:text-gray-500">대응 · Owner</p>
                    <p className="mt-1 whitespace-pre-line text-[15px] leading-[1.7] text-gray-600 dark:text-gray-300">{modal.actions}</p>
                  </div>
                )}
                ...
                {modal.url && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-5 flex items-center justify-center gap-1 rounded-lg bg-gray-900 py-2.5 text-[15px] font-semibold text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-gray-800 active:scale-[.99]"
                  >
                    원문 보기 ↗
                  </a>
                )}
            ...
            >
              <div className="flex items-baseline justify-between border-b border-gray-100 dark:border-gray-800 px-6 pb-3 pt-5">
                <h3 className="text-[19px] font-bold text-gray-900 dark:text-gray-50">{dayList.date.slice(5).replace("-", "/")} 일정</h3>
                <span className="text-[13px] text-gray-400 dark:text-gray-500">{dayList.events.length}건</span>
              </div>
                ...
                {dayList.events.map((e) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => openEvent(e)}
                      className="flex items-start gap-2.5 rounded-lg px-2 py-2.5 text-left transition-colors duration-200 hover:bg-gray-50 dark:hover:bg-gray-900 active:scale-[.99]"
                    >
                      <span className={"mt-[7px] h-1.5 w-1.5 shrink-0 rounded-full " + t.dot} />
                      <span className="min-w-0 flex-1">
                        <span className="block line-clamp-1 text-[15px] font-semibold leading-snug text-gray-900 dark:text-gray-50">{e.importance >= 3 ? "★ " : ""}{e.event}</span>
                        <span className="mt-0.5 block text-[13px] text-gray-500 dark:text-gray-400">{KIND[e.kind] ?? "—"}{e.sourceLabel ? " · " + e.sourceLabel : ""}</span>
                      </span>
                    </button>
                  )
                })}
              ...
              <button
                type="button"
                onClick={closeModal}
                className="m-4 rounded-lg border border-gray-200 dark:border-gray-800 py-2 text-[14.5px] font-medium text-gray-600 dark:text-gray-300 transition-all duration-300 hover:-translate-y-px hover:border-indigo-300 dark:hover:border-indigo-500/40 hover:text-indigo-600 dark:hover:text-indigo-400 active:scale-[.99]"
              >
                닫기
              </button>
            </div>
