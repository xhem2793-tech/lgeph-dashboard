  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[16/10] w-full overflow-hidden border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-950">
        {r.thumb && !err ? (
          ...
          <div className="flex h-full w-full flex-col items-center justify-center gap-1 bg-gradient-to-br from-indigo-500 to-violet-600">
            <span className="text-[15px] font-bold tracking-tight text-white">axlgeph.report</span>
            <span className="text-[12px] font-medium text-white/70">{r.kind}</span>
          </div>
        )}
        <span className="absolute left-3 top-3 rounded-full bg-white/90 dark:bg-gray-900/90 px-2 py-0.5 text-[12px] font-bold text-indigo-700 dark:text-indigo-300 shadow-sm backdrop-blur">{r.kind}</span>
        <span className="absolute right-3 top-3 flex items-center gap-1 rounded-full bg-black/55 px-2 py-0.5 text-[12px] font-semibold text-white backdrop-blur">
      ...
      <div className="flex flex-1 flex-col p-3.5">
        ...
        <h3 className="mt-1.5 line-clamp-2 text-[18px] font-bold leading-snug tracking-tight text-gray-900 dark:text-gray-50 transition-colors duration-300 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">{r.title}</h3>
        <p className="mt-1.5 line-clamp-2 text-[14px] leading-relaxed text-gray-500 dark:text-gray-400">{r.summary}</p>
        {r.so && (
          <p className="mt-2 line-clamp-2 border-l-2 border-indigo-300 dark:border-indigo-500/40 pl-2 text-[13.5px] leading-relaxed text-gray-700 dark:text-gray-200"><b className="font-semibold text-indigo-600 dark:text-indigo-400">시사점</b> {r.so}</p>
        )}
        <div className="mt-auto flex items-center gap-2 border-t border-gray-100 dark:border-gray-800 pt-2.5 text-[12.5px] text-gray-400 dark:text-gray-500">
          {r.sentAt ? (
  ...
  return (
    <main className="w-full px-6 pb-10 pt-4 sm:px-8 lg:px-10">
      ...
      <header className="mb-4 flex flex-wrap items-end justify-between gap-3 border-b border-gray-100 dark:border-gray-800 pb-3" style={{ animation: "fadeUp .5s ease both" }}>
        <div>
          <p className="text-[13px] font-bold uppercase tracking-[.12em] text-indigo-600 dark:text-indigo-400">Market Intelligence</p>
          <h1 className="mt-0.5 text-[26px] font-extrabold tracking-tight text-gray-900 dark:text-gray-50">리포트</h1>
          <p className="mt-0.5 text-[14.5px] text-gray-500 dark:text-gray-400">발간 리포트 원문 · 데이터 출처 검증</p>
        </div>
        <Segmented
          value={tab}
      ...
      {tab === "reports" ? (
        <div key="reports" style={{ animation: "viewIn .42s cubic-bezier(.16,1,.3,1) both" }}>
          {reports === null ? (
            ...
            <div className="rounded-2xl border border-dashed border-gray-200 dark:border-gray-800 py-20 text-center text-[15px] text-gray-400 dark:text-gray-500">아직 발간된 리포트가 없습니다 · <code className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 text-[13px]">npm run publish-report</code> 로 발행</div>
          ) : (
            <>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {reports.map((r, i) => <ReportCard key={r.id} r={r} i={i} />)}
              </div>
              <p className="mt-5 text-[13px] leading-relaxed text-gray-400 dark:text-gray-500">발행 파이프라인(<code className="rounded bg-gray-100 dark:bg-gray-800 px-1 py-px text-[12px]">publish-report</code>)이 리포트를 자동 등록 · 클릭 시 PDF 원문 · 법인 이메일 동시 발송</p>
            </>
          )}
        </div>
      ) : (
