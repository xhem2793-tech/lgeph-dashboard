...
export default function Page() {
  ...
  const clearAll = () => { setFBrand([]); setFType([]); setFProd([]); setFStat([]); setMonthIdx(-1) }
    ...
    const _bcount: Record<string, number> = {}
  if (ads) ads.forEach((x) => { _bcount[x.brand] = (_bcount[x.brand] || 0) + 1 })
  const _brandTop = Object.keys(_bcount).sort((p, q) => _bcount[q] - _bcount[p]).slice(0, 3).join("·")
  const _total = ads ? ads.length : 0
  const _nActive = ads ? ads.filter((x) => x.status === "진행중").length : 0
  const _nEnd = ads ? ads.filter((x) => x.status === "종료예정").length : 0
  const _nNew = ads ? ads.filter((x) => x.status === "새로시작").length : 0
  ...
  return (
    <main className="mx-auto max-w-[1536px] px-4 pb-12 pt-4 sm:px-6">
      ...
      <div className="mb-4 flex items-end justify-between border-b border-gray-100 pb-3"><div><h1 className="text-[19px] font-bold text-gray-900">경쟁사 동향</h1><p className="mt-0.5 text-[12px] text-gray-500">필리핀 가전 경쟁사 광고·프로모 모니터</p></div><span className="text-[11px] text-gray-400">출처 경쟁사 뉴스룸·리테일</span></div>
      ...
      <div className="grid gap-4 lg:grid-cols-[210px_minmax(0,1fr)]">
        ...
        <div className="min-w-0" style={{animation:"fadeUp .5s ease both"}}>
          ...
          {ads === null ? (
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 grid-flow-row-dense">
              <div className="flex overflow-hidden rounded-xl border border-gray-200 bg-gray-50 sm:col-span-2"><div className="w-1 shrink-0 bg-indigo-500" /><div className="flex min-w-0 flex-1 flex-col p-3.5"><div className="flex items-center gap-2"><span className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">상황 인식 · 대응 방향</span><span className="ml-auto text-[9.5px] text-gray-400">AI INTERPRETED</span></div><div className="mt-1 text-[14px] font-bold leading-snug text-gray-900">신규 광고 {_total}건 · {_brandTop} 브랜드 활발</div><div className="mt-1 text-[11.5px] leading-snug text-gray-600"><b className="text-indigo-700">대응</b> 마감임박 {_nEnd}건 · 신규 {_nNew}건 — 프로모 타이밍·대응 점검</div><div className="mt-auto grid grid-cols-3 gap-2 pt-2.5"><div className="rounded-lg bg-white p-2 ring-1 ring-gray-100"><div className="text-[10px] text-gray-500">신규 광고</div><div className="text-[18px] font-bold leading-tight text-gray-900">{_total}<span className="text-[10px] font-medium text-gray-400">건</span></div></div><div className="rounded-lg bg-white p-2 ring-1 ring-gray-100"><div className="text-[10px] text-gray-500">진행중</div><div className="text-[18px] font-bold leading-tight text-gray-900">{_nActive}<span className="text-[10px] font-medium text-gray-400">건</span></div></div><div className="rounded-lg bg-white p-2 ring-1 ring-amber-100"><div className="flex items-center gap-1 text-[10px] text-gray-500"><span className="h-1.5 w-1.5 rounded-full bg-amber-500" />마감임박</div><div className="text-[18px] font-bold leading-tight text-gray-900">{_nEnd}<span className="text-[10px] font-medium text-gray-400">건</span></div></div></div></div></div>
              {shown.map((a, i) => (
