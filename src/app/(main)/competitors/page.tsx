...
function DealsView({ rows, deals }: { rows: PriceRow[] | null; deals: DealRow[] | null }) {
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
      ...
      <div className="flex flex-col gap-2.5 border-b border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-900/40 px-4 py-3">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
          ...
          <div className="relative ml-auto">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="모델·브랜드 검색" className="w-[200px] rounded-full border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 py-1.5 pl-8 pr-3 text-[12.5px] outline-none focus:border-indigo-400 dark:focus:border-indigo-500/50 focus:shadow-[0_0_0_3px_rgba(99,102,241,0.12)]" />
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
      ...
      {/* 테이블 */}
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1200px] table-fixed border-collapse text-[12.5px]">
          <colgroup>
          ...
          <tbody>
            {list.length === 0 ? (
              <tr><td colSpan={11} className="px-4 py-10 text-center text-[12.5px] text-gray-400 dark:text-gray-500">선택한 조건에 해당하는 제품이 없습니다.</td></tr>
              ...
              return (
                <tr key={o.cc + ri} className={"border-b border-gray-100 dark:border-gray-800/60 last:border-0 " + (o.own ? "bg-indigo-50/40 dark:bg-indigo-500/5" : "hover:bg-gray-50 dark:hover:bg-gray-800/40")}>
                  ...
                  <td className="px-3 py-3 text-[12.5px] text-gray-600 dark:text-gray-300">{o.at ? pmShopLabel(o.at) : "—"}</td>
                  <td className="px-3 py-3 text-right"><span className={"text-[14px] tabular-nums font-bold " + (o.own ? "text-indigo-700 dark:text-indigo-300" : isBest ? "text-emerald-700 dark:text-emerald-300" : "text-gray-900 dark:text-gray-50")}>{won(o.net)}</span>{o.list != null && o.list > o.net ? <div className="text-[10.5px] tabular-nums text-gray-400 line-through dark:text-gray-500">{won(o.list)}</div> : null}</td>
                  ...
                  <td className="px-3 py-3 text-right text-[12.5px] tabular-nums">{gapCell(o)}</td>
                  {PTYPES.map((p) => { const pa = promoAt(o, p.k); return <td key={p.k} className="border-l border-gray-100 dark:border-gray-800 px-3 py-3 align-top">{pa ? <><span className={"inline-flex items-center whitespace-nowrap rounded px-1.5 py-0.5 text-[11.5px] font-semibold " + p.cls}>{pa.val}</span>{pa.other ? <span className="mt-0.5 block text-[10px] text-amber-600 dark:text-amber-400">↳ {pmShopLabel(pa.at)}</span> : null}</> : <span className="text-[13px] text-gray-300 dark:text-gray-600">—</span>}</td> })}
                </tr>
              )
            })}
          </tbody>
...
function AnomalyView({ rows, stamp }: { rows: PriceRow[] | null; stamp: string | null }) {
...
function MoversView({ rows }: { rows: PriceRow[] | null }) {
  ...
  const brandsL = React.useMemo(() => {
    const m = new Map<string, number>()
    R.filter((r) => cat === "전체" || r.category === cat).forEach((r) => { if (r.brand) m.set(r.brand, (m.get(r.brand) || 0) + 1) })
    return ["전체", "LG", ...Array.from(m.entries()).filter(([b]) => b !== "LG").sort((a, b) => b[1] - a[1]).map((x) => x[0])]
  }, [R, cat])
  const kw = q.trim().toLowerCase()
  const list = React.useMemo(() => {
    const f = R.filter((r) => r.p0 != null && PM_CATS.includes(r.category) && (cat === "전체" || r.category === cat) && (brand === "전체" || r.brand === brand) && (!kw || (r.brand + " " + r.model + " " + (r.code || "")).toLowerCase().includes(kw)))
    // 전일비 있는 것 우선, 방향순
