...
import { canonCode, isAC, PM_CATS, pmTierOf, pmFormOf, pmFormsFor, pmFormHit, pmSizeList, pmSizeHit, pmSizeBucket, acHpLabel, PM_TIER_BANDS } from "@/lib/classify"
...
function MoversView({ rows, elabels, stamp }: { rows: PriceRow[] | null; elabels: EnergyRow[] | null; stamp: string | null }) {
  const R = rows ?? []
  const cats = React.useMemo(() => { const av = PM_CATS.filter((c) => R.some((r) => r.category === c)); return av.length ? av : PM_CATS }, [R])
  const [cat, setCat] = React.useState("냉장고")
  const [brand, setBrand] = React.useState("전체")
