 ...
 *  헤드라인=롤링 24개월(18실적+6전망) 단일선, 기대 인플레 기준선.
...
/* ============ 롤링 24개월 라인 (실적 단일선 + 전망 점선 + 기대선) — FX차트 비례 ============ */
function FanChart({ actual, forecast, labels, boundary, expect, mounted }: {
  ...
  return (
    <svg viewBox={"0 0 " + W + " " + H} className="w-full" style={{ height: 128 }} aria-hidden>
      ...
      {actual.map((v, i) => v != null && (i === boundary || i % 3 === 0) ? (
        <circle key={i} cx={x(i)} cy={y(v)} r={i === boundary ? 3.4 : 2.2} fill={i === boundary ? "#6366f1" : "#c7d2fe"} stroke="#fff" strokeWidth={i === boundary ? 1.4 : 0}
          style={{ opacity: mounted ? 1 : 0, transition: "opacity .4s ease " + (0.5 + i * 0.025) + "s" }} />
      ) : null)}
      {ticks.map((i) => (
