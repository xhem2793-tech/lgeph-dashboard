    ...
    <main className="mx-auto w-full max-w-[760px] px-6 pb-14 pt-4 sm:px-8" style={{ animation: "fadeUp .5s cubic-bezier(.22,1,.36,1) both" }}>
      ...
      <div className="relative mb-6 overflow-hidden rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50/80 via-white to-fuchsia-50/50 p-5 dark:border-indigo-500/20 dark:from-indigo-500/[0.07] dark:via-gray-950 dark:to-fuchsia-500/[0.05]">
        ...
        <div className="relative flex items-center gap-3.5">
          ...
          <div className="min-w-0">
            <h1 className="text-[19px] font-extrabold tracking-tight text-gray-900 dark:text-gray-50">{me ? "안녕하세요, " + me.name + "님" : "내 설정"}</h1>
            <p className="mt-0.5 truncate text-[12.5px] text-gray-500 dark:text-gray-400">{me ? me.email + " · 로그인 계정 기준으로 저장됩니다" : "로그인 후 개인화가 적용됩니다"}</p>
          </div>
        </div>
      </div>
      ...
      {!me && (
        <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[12.5px] leading-relaxed text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
          현재 로그인 신원을 읽지 못했습니다(미리보기·개발 환경). <b>axlgeph.report</b>에서 로그인하면 계정별로 설정이 저장됩니다. 아래 설정은 이 브라우저에 임시 저장됩니다.
        </div>
      )}
      ...
      <div className="flex flex-col gap-4">
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900/40 sm:p-5">
          <h2 className="text-[14px] font-bold text-gray-900 dark:text-gray-50">기본 시작 페이지</h2>
          <p className="mt-0.5 text-[12px] text-gray-500 dark:text-gray-400">로고를 클릭하면 이동할 나의 홈 화면입니다.</p>
        ...
        {/* 기본 제품 필터 */}
        <section className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900/40 sm:p-5">
          <h2 className="text-[14px] font-bold text-gray-900 dark:text-gray-50">관심 제품(기본 필터)</h2>
          <p className="mt-0.5 text-[12px] text-gray-500 dark:text-gray-400">시장동향·마케팅 등에서 우선 보고 싶은 제품군입니다.</p>
          <div className="mt-3 flex flex-wrap gap-1.5">
            {PROD_OPTS.map((p) => {
              const on = prod === p
        ...
        <section className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900/40 sm:p-5">
            <h2 className="text-[14px] font-bold text-gray-900 dark:text-gray-50">테마</h2>
            <div className="mt-3 inline-flex rounded-full bg-gray-100 p-1 dark:bg-gray-800">
              {([["light", "라이트"], ["dark", "다크"]] as const).map(([k, l]) => (
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900/40 sm:p-5">
            <h2 className="text-[14px] font-bold text-gray-900 dark:text-gray-50">즐겨찾기 지표</h2>
            <p className="mt-2 text-[12px] text-gray-500 dark:text-gray-400">주요지표 리스트에서 ☆로 담은 지표 <b className="tabular-nums text-indigo-600 dark:text-indigo-400">{favN}</b>개</p>
            <a href="/economy" className="mt-3 inline-flex items-center gap-1 text-[12.5px] font-semibold text-indigo-600 hover:underline dark:text-indigo-400">주요지표에서 관리 →</a>
          </div>
        </section>
      </div>

      <p className="mt-6 text-[11px] leading-relaxed text-gray-400 dark:text-gray-500">개인 설정은 <b>로그인 계정(이메일)</b> 기준으로 이 브라우저에 저장됩니다. 다른 기기에서도 동일하게 쓰려면 계정 동기화(서버 저장)를 추후 지원할 수 있습니다.</p>
    </main>
  )
}
