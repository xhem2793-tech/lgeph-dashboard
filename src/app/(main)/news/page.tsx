 ...
 *  규제(Critical/High)는 별도 카드가 아니라 피드 흐름 안에 적색 행으로 끼워 넣는다 — 놓치면 비용이 되기 때문. */
...
/** 지표 칩 — 상승은 적색(원가·물가 상승은 불리), 하락은 녹색 */
...
export default function Page() {
  ...
  return (
    <div className="pb-10">
          ...
          {[7, 30].map((d) => (
            <button
              ...
              className={
                ...
                (days === d ? "bg-indigo-600 text-white" : "text-gray-600 hover:text-indigo-600")
              }
            >
              {d}일
            </button>
      ...
      {brief ? (
        <div className="mb-4 rounded-xl border border-indigo-100 bg-indigo-50/40 p-3">
          <div className="mb-1.5 flex items-center gap-1.5">
            <span className="text-[12px] font-bold text-gray-900">결론 브리핑</span>
            <span className="rounded bg-white px-1.5 py-px text-[10px] font-bold text-indigo-700">
              {brief.status === "approved" ? "CONFIRMED" : "AI INTERPRETED"}
            </span>
            <span className="text-[11px] text-gray-500">{fmtDay(brief.asOf)} 기준</span>
          </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[124px_minmax(0,1fr)_160px]">
        {/* 좌 — 주제 */}
          ...
          <nav className="flex flex-col gap-0.5">
            {TOPICS.map((t) => (
              <button
                ...
                onClick={() => setTopic(t)}
                className={
                  "flex items-center justify-between rounded-md px-2 py-1.5 text-left text-[12px] transition-all duration-200 active:scale-[0.98] " +
                  (topic === t
            ...
            shown.map((f, i) => (
              <React.Fragment key={f.id}>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex gap-3 rounded-lg bg-gray-50 p-3 transition-all duration-300 ease-out hover:-translate-y-1 hover:bg-white hover:shadow-lg hover:shadow-indigo-100"
                >
                  {f.image ? (
        ...
        <aside className="hidden flex-col gap-4 lg:flex">
          <div>
            <p className="mb-1.5 text-[11px] font-bold text-gray-900">뉴스가 움직인 지표</p>
            <div className="flex flex-col gap-1">
              {Object.values(chips).map((c) => (
                <div
                  key={c.k}
                  onMouseEnter={() => setHot(c.k)}
                  onMouseLeave={() => setHot(null)}
                  className={
                    "flex items-center justify-between rounded-md px-1.5 py-1 transition-colors duration-200 " +
                    (hot === c.k ? "bg-indigo-50" : "")
                  }
                >
                  <span className="truncate text-[11px] text-gray-600">{c.label}</span>
                  <span className="num text-[11px] font-semibold text-gray-900">
                    {c.unit === "₱" ? "₱" : ""}
                    {c.value ?? "—"}
                    {c.unit && c.unit !== "₱" ? c.unit : ""}
                  </span>
          ...
          <div className="border-t border-gray-100 pt-3">
            <p className="mb-1.5 text-[11px] font-bold text-gray-900">다가오는 이벤트</p>
            <div className="flex flex-col gap-1.5">
              {events.map((e, i) => (
                <div key={i} className="flex gap-1.5">
                  <span className="num shrink-0 text-[10px] font-bold text-indigo-600">{fmtDay(e.date)}</span>
                  <span className="line-clamp-2 text-[11px] leading-snug text-gray-600">{e.event.split("—")[0]}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="border-t border-gray-100 pt-3">
            <p className="mb-1.5 text-[11px] font-bold text-gray-900">이번 주 칼럼</p>
            <div className="flex flex-col gap-2">
              {posts.map((p) => (
                ...
                >
                  <p className="line-clamp-3 text-[11px] font-medium leading-snug text-gray-700 transition-colors duration-200 group-hover:text-indigo-600">
                    {pick(p.title, p.titleEn)}
                  </p>
                  <p className="mt-0.5 text-[10px] text-gray-500">{p.author ?? "경영기획"}</p>
                </a>
              ))}
            </div>
          </div>
